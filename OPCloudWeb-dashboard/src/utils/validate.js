...
export function isMobile(s) {
  return /^1\d{10}$/g.test(s)
}

// 用户名
export function isUserName(s) {
  return /^[^\d\u4e00-\u9fa5].{2,15}$/.test(s)
}
...
// 身份证
export function isID(idValue) {
  if (!idValue) return false

  var regx = /^[1-9]\d{7}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}$|^[1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}([0-9]|[Xx])$/
  if (!regx.test(idValue)) {
    return false
  }
    ...
    checkArg: function(id) {
      ...
      switch (argType) {
        case 'number':
          ...
          id = id.toString()
          if (id.length > 15) {
            console.log('长数字存在精度问题，请使用字符串传值！')
            return false
          }
          break
        case 'string':
  ...
  // 获取身份证:若是18位获取前17位body及最后一位校验码checkBit,type:18；若是15位，则获取15位身份证body,type:15
  var code = util.checkArg(idValue)
  if (code === false) {
    return false
  }
  ...
  // 18位身份证生日是XXXX-XX-XX，4位年数； 15位身份证生日是XX-XX-XX，2位年数
  var birth =
    code.type === 18 ? code.body.slice(6, 14) : code.body.slice(6, 12)
  // 校验码
  var order = code.body.slice(-3)

  if (
    !(
      util.checkAddr(addr, GB2260) &&
      util.checkBirth(birth) &&
      util.checkOrder(order)
    )
  ) {
    return false
  }
  ...
  for (var j = 0, len = bodyArr.length; j < len; j++) {
...
export function isLPN(s) {
  const regx = /(^[京津冀晋蒙辽吉黑沪苏浙皖闽赣鲁豫鄂湘粤桂琼渝川贵云藏陕甘青宁新]{1}[A-Z0-9]{6}$)|(^[A-Z]{2}[A-Z0-9]{2}[A-Z0-9\u4E00-\u9FA5]{1}[A-Z0-9]{4}$)|(^[\u4E00-\u9FA5]{1}[A-Z0-9]{5}[挂学警军港澳]{1}$)|(^[A-Z]{2}[0-9]{5}$)|(^(08|38){1}[A-Z0-9]{4}[A-Z0-9挂学警军港澳]{1}$)/
...
// 道路运输证号
export function isOpraLicNo(s) {
  return /^([0-9a-zA-Z]{12})|([0-9a-zA-Z]{19})$/.test(s)
}
 ...
 * 判断是否为小数
 */
export  function checkNumord(num, type) {
  let regName = /[^\d.]/g;
  if (type == 1) {
    if (!regName.test(num)) return false
  } else if (type == 2) {
