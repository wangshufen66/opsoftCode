...
export default {
  ...
  setting: {
    ...
    dataAPI:
      "https://mock.kunteng.org.cn/mock/5ca2cba34918866472494a14/chartMap", // API拉取地址
        ...
        mapData: [
          {
            ...
            tooltipValue: [46, 26, 33],
          },
          {
            name: "沧州市",
            value: Math.round(Math.random() * 100),
            tooltipValue: [46, 50, 14],
          },
          {
            name: "邢台市",
            value: Math.round(Math.random() * 100),
            tooltipValue: [46, 56, 3],
          },
          {
            name: "承德市",
            value: Math.round(Math.random() * 100),
            tooltipValue: [46, 62, 65],
          },
          {
            name: "张家口市",
            value: Math.round(Math.random() * 100),
            tooltipValue: [46, 35, 29],
          },
          {
            name: "保定市",
            value: Math.round(Math.random() * 100),
            tooltipValue: [46, 25, 12],
          },
          {
            name: "衡水市",
            value: Math.round(Math.random() * 100),
            tooltipValue: [46, 15, 4],
          },
          {
            name: "唐山市",
            value: Math.round(Math.random() * 100),
            tooltipValue: [46, 33, 9],
          },

          {
            name: "吕梁市",
            value: Math.round(Math.random() * 100),
            tooltipValue: [32, 33, 9],
          },
          {
            name: "朔州市",
            value: Math.round(Math.random() * 100),
            tooltipValue: [46, 88, 54],
          },
          {
            name: "晋中市",
            value: Math.round(Math.random() * 100),
            tooltipValue: [65, 33, 79],
          },
          {
            name: "运城市",
            value: Math.round(Math.random() * 100),
            tooltipValue: [46, 47, 9],
          },
  ...
  computed: {
    dataSeries() {
      if (
        ...
        let data = this.dynamicData[this.val.keyPrimary][
          this.val.keyTarget
        ].map((item) => {
          return {
            name: item.name,
      ...
      return {
        ...
        tooltip: {
          ...
          formatter: function (params) {
            if (
              params.value &&
              params.data.tooltipValue &&
              params.data.tooltipValue.length > 0
            ) {
  ...
  methods: {
    ...
    tooltipStr(item) {
      ...
      for (let i in item.tooltipValue) {
        str += `${this.dynamicData[this.val.keyPrimary].tooltipName[i]}：${
          item.tooltipValue[i]
        } ${this.dynamicData[this.val.keyPrimary].tooltipUnits[i]}</br>`;
      }
      return str;
    },
    getProJSON(str) {
