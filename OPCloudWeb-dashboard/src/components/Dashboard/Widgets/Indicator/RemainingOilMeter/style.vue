      ...
      <div class="panel-item-new">
        ...
        <div v-show="isShowProgress" class="panel-item-new-set">
          <div class="panel-row">
            ...
            <div>
              <input v-model="activeElement.seriesBgColor" type="color">
            </div>
          </div>
          <div style="display: none;" class="panel-row" flex>
            <div class="panel-label">进度条渐变</div>
            <div class="panel-value">
              <label class="form-switch">
                <input v-model="activeElement.isLinear" type="checkbox">
                <i class="form-icon" />
              </label>
            </div>
          </div>
          <div v-show="activeElement.isLinear" class="panel-row">
            <div class="panel-label">配置颜色</div>
            <div class="panel-setcolor">
              <div v-for="(item,index) in activeElement.lgArr" :key="index" class="panel-setcolor-item">
                <div class="panel-value-new">{{ item.color }}</div>
                <div>
                  <input v-model="item.color" type="color">
                  <input v-if="activeElement.lgArr.length > 1" v-model="item.offset" type="number" step="0.1" min="0" max="1" class="radiusInp">
                </div>
          ...
          <div v-show="!activeElement.isLinear" class="panel-row">
            ...
            <div>
              <input v-model="activeElement.seriesColors[0]" type="color">
            </div>
          </div>
