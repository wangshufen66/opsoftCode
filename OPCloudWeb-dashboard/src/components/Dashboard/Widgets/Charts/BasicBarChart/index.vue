<template>
  ...
  >
    <v-echart :id="chartId" :options="options" autoresize class="ffff" />
  </div>
</template>
...
export default {
  ...
  setting: {
    ...
    dataAPI:
      "https://easy-mock.com/mock/5c7ce20ccdc04f0e04185d9b/example/quickv/api/demo", // API拉取地址
  ...
  watch: {
    ...
    "val.autoToolTip": function (val) {
      this.drawLine(val ? this.val.autoToolTipTime : 0);
    },
  ...
  computed: {
    categories() {
      if (
        ...
        return this.dynamicData[this.val.keyPrimary][this.val.keyTarget].map(
          (item) => {
    ...
    options() {
      return {
        ...
        series: [
          {
            name: this.val.toolTipTitle,
            type: "bar",
            barWidth: this.val.seriseBarWidth, // 柱形图宽度
            itemStyle: {
              normal: {
                barBorderRadius: [
                  this.val.seriseRadiuses[0] || this.val.seriseRadiuses[0] + 1,
                  this.val.seriseRadiuses[1] || this.val.seriseRadiuses[1] + 1,
                  this.val.seriseRadiuses[2] || this.val.seriseRadiuses[2] + 1,
                  this.val.seriseRadiuses[3] || this.val.seriseRadiuses[3] + 1,
                ], // 柱形图圆角
  ...
  beforeMount() {
    if (this.val.autoToolTip) {
      this.drawLine(this.val.autoToolTipTime);
    }
  },
  mounted() {
    this.options.xx = [];
  ...
  methods: {
    drawLine(time) {
      ...
      this.timer = autoToolTip.autoHover(
        myChart,
        this.options,
        this.dataSeries.length,
        time
      );
    },
  },
