<template>
  <div :style="{
    ...
    <div class="table-title table-tr" :style="{
        height: val.showTh ? val.thHeight + 'px' : '0px',
        textAlign: val.tbAlign
      }">
      <div v-show="val.showTh && val.showNum" class="table-th" :style="{
        color: val.thColor,
      ...
      <div v-for="(item, index) in dynamicData.data.columns" :key="item.id" class="table-th" :style="{
          color: val.thColor,
          fontSize: val.thFontSize + 'px',
          background: val.thBgColor,
          display: val.showTh ? 'flex' : 'none',
    ...
    <div :style="{
      ...
      <!-- 表内容块 -->
      <div v-for="(item, index) in dynamicData.data.rows.slice(0, val.dataLength)" :key="item.id" class="table-tr">
        <div v-show="val.showNum" class="table-td table-td-num">
          <span :style="{
        ...
        <div v-for="(value, key, index) in item" :key="item[value]" :style="{
          color: val.tbColor,
...
export default {
  ...
  setting: {
    ...
    dataAPI:
      'https://easy-mock.com/mock/5c7ce20ccdc04f0e04185d9b/example/echart/basictable', // API拉取地址
  ...
  watch: {
    dynamicData() {
      console.log(this.val.dataLength);
