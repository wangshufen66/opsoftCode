<template>
  <div id="viewport" class="holder viewport-scroll" :style="preview ? screenSize : {}">
    <ruler-tool :ruler-toggle="!preview" :width="width" :height="height" :zoom="zoom" :content-layout="{ left: 0, top: 0 }" :is-scale-revise="true">
      <div :style="{
          ...
          transform: preview
            ? 'scale(' + 1 / dpr + ')'
            : 'scale(' + zoom / 100 + ')',
        }" class="screen" id="viewport-screen" @dblclick="replaceImage">
        <drag-select :zoom="zoom" :minHeight="height" v-model="selectedList" ref="dragSelect">
          <!-- 组件 -->
          <component v-for="val in widgetStore" :is="val.type" :data-title="val.name" :class="{ 'g-active': isSelected(val.uuid) }" :key="val.uuid" :val="val" :h="height" :w="width" :data-type="val.type" :data-uuid="val.uuid" class="layer">
            <component v-for="child in getChilds(val.name)" :is="child.type" :data-title="child.name" :class="{ 'g-active': isSelected(child.uuid) }" :key="child.uuid" :val="child" :h="height" :w="width" :data-type="child.type" :data-uuid="child.uuid" class="layer layer-child" />
          </component>
        </drag-select>
...
export default {
  ...
  methods: {
    ...
    handleSelection(e) {
      ...
      if (type) {
        uuid =
          target.getAttribute('data-uuid') || curDiv.getAttribute('data-uuid');

        // 设置选中元素
        if (
          this.$vpd.state.multiSelect &&
          this.$vpd.state.multiSelectCols.indexOf(uuid) > -1
        ) {
          this.initmovement(e);
        } else {
