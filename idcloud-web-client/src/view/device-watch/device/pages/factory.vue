<template>
  <div class="device-container">
      ...
      <div class="up-left">
        ...
        <div class="up-left-content-wrapper" v-loading="topCard.loading">
          ...
          <div class="up-left-card">
            <mini-card
              v-for="(item,index) in topCard.miniCard"
              :key="item.name"
              :miniCardData="item"
            >
            </mini-card>
    ...
    <div class="down-wrapper">
      ...
      <el-tabs v-model="tabValue" type="card" @tab-click="handleClickTab">
        <el-tab-pane label="车间" name="department">
          <div v-if="downStyle" v-loading="downCardDepartment.loading">
            <div
              v-if="downCardDepartment.dataForShow.length === 0"
              class="placeholder-wrapper"
            >
              <img src="@/view/device-watch/device/assets/300-220.png"/>
              <span style="margin-left: 12px">暂无车间信息！</span>
            </div>
            <div v-else class="down-card-department-wrapper">
              <shop-card
                v-for="item in downCardDepartment.dataForShow"
                :key="item.id"
                :cardData="item"
                :color-list="colorIconList"
              ></shop-card>
            </div>
          </div>
          <el-table
            height="370px"
            v-else
            :data="downCardDepartment.tableDataForShow"
          ...
          >
            ...
            ></el-table-column>
            <el-table-column
              v-for="item in downCardDepartment.tableColumn"
              :key="item.id"
              :prop="item.key"
              :label="item.title"
            ></el-table-column>
        ...
        <el-tab-pane label="公共设备" name="device">
          <div v-if="downStyle" v-loading="downCardDevice.loading">
            ...
            >
              <img src="@/view/device-watch/device/assets/300-220.png"/>
              <span style="margin-left: 12px">暂无公共设备信息！</span>
            </div>
            <div v-else class="down-card-department-wrapper">
              <device-card
          ...
          >
            ...
            >
              <template slot-scope="scope">
                <div
                  @click="handleClickWarning(scope.row.relation)"
                  v-if="item.key == 'warningStatus'"
                >
                  <span
...
<script>
  ...
  export default {
    ...
    components: {downCard, deviceCard, miniCard, shopCard},
    props: {
      treeData: {
        type: Object,
        default() {
    ...
    data() {
      return {
        ...
        downCardDepartment: {
          ...
          tableColumn: [
            {title: "车间名称", key: "name"},
            {title: "设备总数量(台)", key: "all"},
            {title: "运行设备数(台)", key: "run"},
            {title: "离线设备数(台)", key: "off"},
            {title: "待机设备数(台)", key: "wait"},
            {title: "故障设备数(台)", key: "break"},
            {title: "调机设备数(台)", key: "tune"},
            // 空载状态 =》 下方表格
            // {title: "空载设备数(台)", key: "empty"},
          ],
        },
        downCardDevice: {
          loading: false,
    ...
    methods: {
      async handleClickWarning(item) {
        try {
          let res = await Promise.resolve(getWarningLog(JSON.parse(item)));
      ...
      pageChange(val) {
        this.downCardDepartment.tableDataForShow = this.downCardDepartment.tableData.slice(
          (val - 1) * 8,
          (val - 1) * 8 + 8
        );
        this.downCardDepartment.dataForShow = this.downCardDepartment.data.slice(
          (val - 1) * 8,
          (val - 1) * 8 + 8
        );

        this.downCardDevice.tableDataForShow = this.downCardDevice.tableData.slice(
          (val - 1) * 8,
          (val - 1) * 8 + 8
        );
        this.downCardDevice.dataForShow = this.downCardDevice.data.slice(
          (val - 1) * 8,
          (val - 1) * 8 + 8
        );
      },
      async getPointData() {
        const param = {orgId: this.orgId, id: this.treeData.id, type: "factory"};
      ...
      async getDepartment() {
        this.downCardDepartment.tableData = [];
        const param = {orgId: this.orgId, id: this.treeData.id};
        this.downCardDepartment.loading = true;
        let res = await getWorkshopListByContainer(param);
        if (res.status === 200 && res.data.code === 200) {
          this.downCardDepartment.data = res.data.data;
          res.data.data.forEach((i) => {
            let pushItem = {
              name: i.ctrName,
              all: i.statistics.reduce((sum, curr) => {
                return sum + curr.y
              }, 0),
              run: i.statistics.findIndex((item) => item.x === "1") >= 0 ?
                i.statistics[i.statistics.findIndex((item) => item.x === "1")].y : 0,
              off: i.statistics.findIndex((item) => item.x === "0") >= 0 ?
                i.statistics[i.statistics.findIndex((item) => item.x === "0")].y : 0,
              wait: i.statistics.findIndex((item) => item.x === "2") >= 0 ?
                i.statistics[i.statistics.findIndex((item) => item.x === "2")].y : 0,
              break: i.statistics.findIndex((item) => item.x === "4") >= 0 ?
                i.statistics[i.statistics.findIndex((item) => item.x === "4")].y : 0,
              tune: i.statistics.findIndex((item) => item.x === "3") >= 0 ?
                i.statistics[i.statistics.findIndex((item) => item.x === "3")].y : 0,
            };
            this.downCardDepartment.tableData.push(pushItem);
          });
          this.page.total = this.downCardDepartment.tableData.length;
          this.downCardDepartment.tableDataForShow = this.downCardDepartment.tableData.slice(
            0,
            8
          );
          this.downCardDepartment.dataForShow = this.downCardDepartment.tableData.slice(
            0,
            8
          );
        }
        this.downCardDepartment.loading = false;
      },
      async getDevice() {
      ...
      handleClickTab() {
        ...
        switch (this.tabValue) {
          case "department":
            ...
            break;
          case "device":
            this.page.total = this.downCardDevice.tableData.length;
            break;
          default:
            break;
        }
