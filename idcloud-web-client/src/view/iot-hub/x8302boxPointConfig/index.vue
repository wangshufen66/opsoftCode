<template>
  <div id="x8302box-point-config-container" style="display: flex;flex-direction: row;margin-left:-27px">
    <id-tree class="main-tree" ref="idTree" v-loading="treeLoading" :treeData="treeData" :highlight-current="highlightCurrent" :default-expand-all="false" :idConfig="treeActionBar" @nodeClick="nodeSelectFunc"></id-tree>
    <div class="right-area-container" style="margin-left: 10px;flex-grow: 1;">
      <div class="parent-wrapper" v-if="formIntent === 'parent'">
        <div class="parent-header-wrapper">
          <span>{{containerInfosData.name}}</span>
          <div style="display: flex">
            <el-button v-if="formIntent === 'parent'" @click="goAddInfoPoint" size="small" type="primary">添加
            </el-button>
            <div @click="findGroupListFunc" style="height: 32px; line-height: 32px; margin-left: 8px; cursor: pointer; font-size: 14px; color: #266DEB;">
              <i class="el-icon-refresh-right">刷新</i>
            </div>
          </div>
        </div>
        <div class="parent-body-wrapper">
          <el-card v-for="(item,index) in containerInfosData.box8302PointList" :key="index" class="body-card">
      ...
      <div class="else-wrapper" v-else-if="formIntent === 'child' || formIntent === 'add'">
        <div class="else-header-wrapper">
          <span v-if="formIntent === 'child'">{{containerInfosData.name}}</span>
          <div v-else class="new-point-wrapper">
            <el-button @click="formIntent = 'parent'" class="transparent-btn" size="small" icon="el-icon-arrow-left"></el-button>
            <span>新建数据点</span>
          </div>
          <div class="btn-wrapper" style="display: flex">
            <el-button-group v-if="formIntent === 'child'">
              <el-button @click="goEditInfoPoint" size="small" type="primary">保存
              </el-button>
              <el-button style="border: 1px solid #409EFF;" @click="goDelInfoPoint" size="small">删除
              </el-button>
            </el-button-group>
        ...
        <el-form label-position="right" class="else-form" ref="theForm" label-width="100px" :rules="formRule" :model="formValidate">
          <div v-for="item in formColumn" :key="item.key" class="form-inner">
            <el-form-item v-if="item.key !== 'mark'" :label="item.title" label-width :prop="item.key" class="form-item">
              <el-input v-if="item.type === 'text'&&formIntent === 'add'" class="input" v-model="formValidate[item.key]"></el-input>
              <el-input v-else-if="item.type === 'text'&&formIntent === 'child'&&item.key!=='name'" class="input" v-model="formValidate[item.key]"></el-input>
              <el-input v-else-if="item.key ==='number' && formIntent === 'child'" class="input" v-model="formValidate[item.key]" disabled></el-input>
              ...
              <el-input v-else-if="item.key ==='name' && formIntent === 'child'" class="input" v-model="formValidate[item.key]"></el-input>
              <el-input class="input" v-else-if="item.key === 'number' && formIntent === 'add'" readonly v-model="formValidate[item.key]">
                <el-button @click="dialogSelectClick(item)" style="width: 40px" icon="el-icon-more" slot="append"></el-button>
              </el-input>
            ...
            <el-form-item class="text-area-wrapper" :label="item.title" label-width :prop="item.key" v-else>
              <el-input type="textarea" v-if="item.type === 'textarea'" class="textarea" v-model="formValidate[item.key]"></el-input>
            </el-form-item>
          </div>
        </el-form>
      </div>
    ...
    <el-dialog @open="tableFormOpenCallback" top="60px" @closed="tableFormCloseCallback" :title="tableDialogTitle" :visible.sync="tableInFormShow">
      <div class="table-search-wrapper">
        <el-select class="search-select-style" size="small" v-model="tableInFormSelected" placeholder="请选择">
          <el-option v-for="item in formTableColumn" v-show="item.type === 'text'" :key="item.key" :label="item.title" :value="item.key"></el-option>
        </el-select>
        <el-input clearable class="search-input-style" size="small" v-model="tableInFormInput"></el-input>
        <el-button @click="handleTableInFormSearch" class="search-btn-style" type="primary" icon="el-icon-search" circle></el-button>
      </div>
      ...
      <!--&gt;</el-table-column>-->
      <!--</el-table>-->
      <Table border id="i-view-table" :columns="formTableColumn" :data="formTableData" @on-row-dblclick="handleFormTableDbClick">
      </Table>
      <div class="pagination-style">
        <el-pagination @current-change="handleTableInFormChangePage" :current-page="tableInFormCurrentPage" background layout="total,prev, pager, next" :total="tableInFormTotal"></el-pagination>
      </div>
    </el-dialog>
  </div>
</template>
...
import {
  ...
  updateOne,
  getInfoPoint,
} from '@/api/iot-hub/x8302boxPointConfig';
      ...
      try {
        this.$refs.theForm.validate(async (valid) => {
          if (valid) {
            const param = {
              orgId: this.orgId,
              box8302Id: this.containerInfosData.box8302Id,
              ...this.formValidate,
...
#x8302box-point-config-container {
