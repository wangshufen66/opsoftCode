      ...
      return {
        ...
        tableDetailForm: {
          ...
          idForm: {
            ...
            rules: {
              number: [{required: true, message: "cnc品牌型号代码不能为空", trigger: ['blur', 'change']}],
              cncBrand: [{required: true, message: "cnc品牌不能为空", trigger: ['blur', 'change']}],
              cncModel: [{required: true, message: "cnc型号不能为空", trigger: ['blur', 'change']}],
              name: [{required: true, message: "cnc品牌型号描述不能为空", trigger: ['blur', 'change']}],
            },
            component: [
              //需要class化
              {
      ...
      initListDetail(index, row, callBack) {
        if (row) {
          ...
          this.formList[0].form.data = {
            cncBrandId: this.tableDetailForm.idForm.data.id,//表单Id
            id: "",
            collectionNumber: "",//云盒代码
            pointCodeId: 0,//数据点ID
            pointCodeNumber: "",//数据点代码
