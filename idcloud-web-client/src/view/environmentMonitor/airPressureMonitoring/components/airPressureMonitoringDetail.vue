<template>
  <div id="container"
    ...
    <table-el
      ...
      :table-tag="tabKey"
      :serverPage="tabs.airPressureMonitoringDetail.serverPage"
      :table-uni-key="tabs.airPressureMonitoringDetail.tableUniKey"
      :vuex-first-key="tabs.airPressureMonitoringDetail.vuexFirstKey"
      :vuex-second-key="tabs.airPressureMonitoringDetail.vuexSecondKey"
      :columns-std="tabs.airPressureMonitoringDetail.columnsStd"
      :table-config="tabs.airPressureMonitoringDetail.tableConfig"
...
<script>
  ...
  import {detail} from '@/api/environmentMonitor/airPressureMonitor'
  import moment from 'moment'

  export default {
    name: "airPressureMonitoringDetail",
    ...
    data() {
      ...
      return {
        detailData: '',
        tabKey: 'airPressureMonitoringDetail',
        buttonParentName: 'idc_airPressureMonitoringDetail', // 所有按钮对应的菜单名称（后台数据库存放的数据决定的）
        ...
        tableHeight: '300',
        pageHeight: 0,
        activeTab: 'airPressureMonitoringDetail',  // 当前激活的页签(对象的key值)
        ...
        tabs: {
          airPressureMonitoringDetail: {
            ...
            vuexFirstKey: 'airPressureMonitoringDetail', // 表格组件获取 vuex 数据的 一级Key
            vuexSecondKey: 'airPressureMonitoringDetail', // 表格组件获取 vuex 数据的 二级Key
    ...
    methods: {
      ...
      convertResponseToLineChartData(respData) {
        const tmp = respData
        respData = respData['pressureUseList']
      ...
      setLineChartMin(chartData) {
        ...
        this.$refs['airPressureLineChart'].chartData.yAxisData.yAxisMin = min
      },
      ...
      setLineChartMax(chartData) {
        ...
        // console.log('max = ' + max)
        this.$refs['airPressureLineChart'].chartData.yAxisData.yAxisMax = max
      },

      setLineChartMinMax(chartData) {
        this.$refs['airPressureLineChart'].chartData.yAxisData.yAxisSplitNumber = 5
      ...
      refreshTabTable(tableTag, curPage, curPageSize, callback) {
        ...
        // 表格的唯一标识和页签的唯一key相同
        switch (tableTag) {
          case 'airPressureMonitoringDetail':
      ...
      requestDetailData(curPage, curPageSize, callback) {
        ...
        //   "pressureNum": this.detailData['pressureNum'],
        ...
        this.tabs.airPressureMonitoringDetail.tableConfig.loading = true
        Promise.all(promiseArray).then(response => {
          // console.log(JSON.stringify(response))
          let [electricMonitoringResponse] = response

          if(electricMonitoringResponse.data.data.tableName!==undefined)
            electricMonitoringResponse.data.data.pressureUseList=electricMonitoringResponse.data.data.pointHisList
          ...
          this.tabs.airPressureMonitoringDetail.tableConfig.loading = false
        })
      },
      ...
      receiveChartData(response) {
        ...
        if (response.code !== 200) {
          ...
          // this.tabs.airPressureMonitoringDetail.tableConfig.columnsStd = this.$store.state['airPressureMonitoringDetail'].moColumns.columnsStd
          // this.tabs.airPressureMonitoringDetail.tableConfig.columns = this.tableColumns
          // this.tabs.airPressureMonitoringDetail.tableConfig.countTotal = response.data.total
          // console.log('数据：' + JSON.stringify(tableData))

          // 将后台数据转换成前端可用于显示折线统计图图形的数据
          let chartData = this.convertResponseToLineChartData(response.data)

          this.$refs[`airPressureLineChart`].drawLine(chartData.xAxisData, chartData.seriesData)
        }
      },
      ...
      receiveMoTableData(response, callback) {
        ...
        if (response.code !== 200) {
          ...
          let tableData = [...response.data['pressureUseList']]
          ...
          // this.tabs.airPressureMonitoringDetail.tableConfig.columnsStd = this.$store.state['airPressureMonitoringDetail'].moColumns.columnsStd
          this.tabs.airPressureMonitoringDetail.tableConfig.columns = this.tableColumns
          this.tabs.airPressureMonitoringDetail.tableConfig.countTotal = 0 //response.data.total
          // console.log('数据：' + JSON.stringify(tableData))
          // console.log(tableData)
          // console.log(this.tableColumns)
          // console.log(this.$refs[`tableEl_${this.tabKey}`])
          if (this.$refs[`tableEl_${this.tabKey}`]) {
            this.$refs[`tableEl_${this.tabKey}`].drawTableData(this.tableColumns, tableData)
          }
          ...
          if (callback) {
            ...
            if (this.$refs[`tableEl_${this.tabKey}`]) {
              this.$refs[`tableEl_${this.tabKey}`].updateSelectedRowsToVuex(tableData, [])
            }
          }
      ...
      requestElectricMonitoringDataById(data) {
        ...
        // this.$refs[`airPressureLineChart`].drawLine([], [])
        // this.$refs[`tableEl_airPressureMonitoringDetail`].drawTableData(this.tableColumns, [])
        // this.requestChartData()
        this.refreshTabTable('airPressureMonitoringDetail', this.tabs.airPressureMonitoringDetail.tableConfig.curPage, this.tabs.airPressureMonitoringDetail.tableConfig.pageSize, null)
      },
