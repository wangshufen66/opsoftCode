<template>
  <div id="container"
    ...
    <div id="listTabText" style="margin-bottom: -30px">
      <!--<span style="font-size: large" v-text="title+' 采集信息'"></span>-->
    ...
    <table-el
      ...
      :table-tag="tabKey"
      :serverPage="tabs.waterUsageMonitoringDetail.serverPage"
      :table-uni-key="tabs.waterUsageMonitoringDetail.tableUniKey"
      :vuex-first-key="tabs.waterUsageMonitoringDetail.vuexFirstKey"
      :vuex-second-key="tabs.waterUsageMonitoringDetail.vuexSecondKey"
      :columns-std="tabs.waterUsageMonitoringDetail.columnsStd"
      :table-config="tabs.waterUsageMonitoringDetail.tableConfig"
...
<script>
  ...
  import {detail} from '@/api/environmentMonitor/waterUsageMonitor'
  import moment from 'moment'

  export default {
    name: "waterUsageMonitoringDetail",
    ...
    data() {
      ...
      return {
        detailData: '',
        tabKey: 'waterUsageMonitoringDetail',
        buttonParentName: 'idc_waterUsageMonitoringDetail', // 所有按钮对应的菜单名称（后台数据库存放的数据决定的）
        ...
        tableHeight: '300',
        pageHeight: 0,
        activeTab: 'waterUsageMonitoringDetail',  // 当前激活的页签(对象的key值)
        ...
        // }],
        tabs: {
          waterUsageMonitoringDetail: {
            label: '用水详细信息',
            type: 'normal',
            show: false,
            tableUniKey: 'id', // 作为表格数据唯一识别的字段名（即 属性 key）
            vuexFirstKey: 'waterUsageMonitoringDetail', // 表格组件获取 vuex 数据的 一级Key
            vuexSecondKey: 'waterUsageMonitoringDetail', // 表格组件获取 vuex 数据的 二级Key
    ...
    methods: {
      ...
      convertResponseToLineChartData(respData) {
        const tmp = respData
        respData = respData['waterUseList']
      ...
      setLineChartMin(chartData) {
        ...
        this.$refs['waterUsageLineChart'].chartData.yAxisData.yAxisMin = min
      },
      ...
      setLineChartMax(chartData) {
        ...
        // console.log('max = ' + max)
        this.$refs['waterUsageLineChart'].chartData.yAxisData.yAxisMax = max
      },

      setLineChartMinMax(chartData) {
        this.$refs['waterUsageLineChart'].chartData.yAxisData.yAxisSplitNumber = 5
      ...
      refreshTabTable(tableTag, curPage, curPageSize, callback) {
        ...
        // 表格的唯一标识和页签的唯一key相同
        switch (tableTag) {
          case 'waterUsageMonitoringDetail':
      ...
      requestDetailData(curPage, curPageSize, callback) {
        ...
        //   "waterNum": this.detailData['waterNum'],
        ...
        this.tabs.waterUsageMonitoringDetail.tableConfig.loading = true
        Promise.all(promiseArray).then(response => {
          // console.log('echarts response = ' + JSON.stringify(response))
          let [electricMonitoringResponse] = response

          if (electricMonitoringResponse.data.data.tableName !== undefined)
            electricMonitoringResponse.data.data.waterUseList = electricMonitoringResponse.data.data.pointHisList

          // 接收页签的表格数据（后台数据）
          this.receiveChartData(electricMonitoringResponse)
          // 接收页签的表格数据（后台数据）
          this.receiveMoTableData(electricMonitoringResponse, callback)
        }).catch(response => {
          ...
          this.tabs.waterUsageMonitoringDetail.tableConfig.loading = false
        })
      },
      ...
      receiveChartData(response) {
        ...
        if (response.code !== 200) {
          ...
          // this.tabs.temperatureMonitoringDetail.tableConfig.columnsStd = this.$store.state['temperatureMonitoringDetail'].moColumns.columnsStd
          // this.tabs.temperatureMonitoringDetail.tableConfig.columns = this.tableColumns
          // this.tabs.temperatureMonitoringDetail.tableConfig.countTotal = response.data.total
          // console.log('数据：' + JSON.stringify(tableData))

          // 将后台数据转换成前端可用于显示折线统计图图形的数据
          let chartData = this.convertResponseToLineChartData(response.data)

          this.$refs[`waterUsageLineChart`].drawLine(chartData.xAxisData, chartData.seriesData)
        }
      },
      // 接收<生产任务单>页签的表格数据（后台数据）
      receiveMoTableData(response, callback) {
        ...
        if (response.code !== 200) {
          ...
          let tableData = [...response.data['waterUseList']]
          ...
          tableData.map(item => {
            ...
            item['consumptionTitle'] = '用水量'
            item['consumptionUnit'] = `m³`
            item['voltage'] = 220 // todo: 电压缺字段
            item['electricity'] = ((!item['num']) ? 0 : item['num'] / 100 / 220).toFixed(2) // todo: 电流缺字段
            item['createTime'] = item['time']
            item['frequency'] = '5s'
          })

          // this.tabs.temperatureMonitoringDetail.tableConfig.columnsStd = this.$store.state['temperatureMonitoringDetail'].moColumns.columnsStd
          this.tabs.waterUsageMonitoringDetail.tableConfig.columns = this.tableColumns
          this.tabs.waterUsageMonitoringDetail.tableConfig.countTotal = 0 //response.data.total
          // console.log('数据：' + JSON.stringify(tableData))

          this.$refs[`tableEl_waterUsageMonitoringDetail`].drawTableData(this.tabs.waterUsageMonitoringDetail.tableConfig.columns, tableData)
          // 绘制生产工序的表格
          // console.log(JSON.stringify(response))
          ...
          if (callback) {
            ...
            this.$refs['tableEl_waterUsageMonitoringDetail'].updateSelectedRowsToVuex(tableData, [])
          }

          // 每次刷新此表格后，恢复相应的表格刷新意图
          // this.$store.dispatch('set_ElectricMonitoringDetailTableRefreshIntent', false)
        }
      },
      ...
      handleTransmitSelectedTimes(name) {
        ...
        this.$refs[name].validate((valid) => {
          if (valid) {
            this.requestElectricMonitoringDataById(this.detailData)
          } else {
            this.$message.error('验证失败，请根据提示修改内容!');
          }
        })
      },
      requestElectricMonitoringDataById(data) {
        this.detailData = data
        this.$refs[`waterUsageLineChart`].drawLine([], [])
        this.$refs[`tableEl_waterUsageMonitoringDetail`].drawTableData(this.tableColumns, [])

        // this.requestChartData()
        this.refreshTabTable('waterUsageMonitoringDetail', this.tabs.waterUsageMonitoringDetail.tableConfig.curPage, this.tabs.waterUsageMonitoringDetail.tableConfig.pageSize, null)
      },
