<template>
  <div>
    ...
    <table-el
      ...
      :serverPage="tabs.waterCount.serverPage"
      :table-uni-key="tabs.waterCount.tableUniKey"
      :vuex-first-key="tabs.waterCount.vuexFirstKey"
      :vuex-second-key="tabs.waterCount.vuexSecondKey"
      :columns-std="tabs.waterCount.columnsStd"
      :table-config="tabs.waterCount.tableConfig"
      :hasAddButton="tabs.waterCount.tableConfig.hasAddButton"
...
<script>
  ...
  import {usageStatistics} from "@/api/environmentMonitor/waterUsageMonitor";
  ...
  export default {
      ...
      return {
        ...
        tabs: {
          waterCount: {
            ...
            vuexFirstKey: 'waterCount', // 表格组件获取 vuex 数据的 一级Key
            vuexSecondKey: 'waterCount', // 表格组件获取 vuex 数据的 二级Key
    ...
    mounted(){
      this.requestData(this.tabs.waterCount.tableConfig.curPage, this.tabs.waterCount.tableConfig.pageSize);
    },
    methods: {
      //表格界面获取数据
      ...
      requestData(curPage, curPageSize, callback) {
        this.tabs.waterCount.tableConfig.loading = true;
        const param = {
          orgId: this.$store.state.user.userInfo.orgId,
          type:this.type.vModel,
          time:this.type.vModel==='2'?undefined:this.queryTime,
        };
        if(this.tabs.waterCount.tableConfig.search!=='')
          param[this.$refs[`tableEl_waterCount`].searchTextContentConfig.value]=this.tabs.waterCount.tableConfig.search;
        let promiseArray = [
        ...
        Promise.all(promiseArray).then(response => {
          ...
          this.tabs.waterCount.tableConfig.loading = false;
        })
      },
      //接受到的数据处理
      receiveTableData(response) {
        ...
        if (response.code !== 200) {
          ...
          this.tabs.waterCount.tableConfig.columns = this.detailViewTableColumns;
          this.tabs.waterCount.tableConfig.countTotal = tableData.length;
          //前台分页处理（操作空间）
          // this.tabs.waterCount.tableConfig.curPage
          // this.tabs.waterCount.tableConfig.pageSize
          let autoData=tableData.slice(
            (this.tabs.waterCount.tableConfig.curPage-1)*this.tabs.waterCount.tableConfig.pageSize,
            this.tabs.waterCount.tableConfig.curPage*this.tabs.waterCount.tableConfig.pageSize
          );
          //
          this.$refs[`tableEl_waterCount`].drawTableData(this.detailViewTableColumns, autoData);
        }
      },
