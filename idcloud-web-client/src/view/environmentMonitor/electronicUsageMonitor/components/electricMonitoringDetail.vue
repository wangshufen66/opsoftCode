<template>
  <div id="container"
    ...
    <table-el
      ...
      :table-tag="tabKey"
      :serverPage="tabs.electricMonitoringDetail.serverPage"
      :table-uni-key="tabs.electricMonitoringDetail.tableUniKey"
      :vuex-first-key="tabs.electricMonitoringDetail.vuexFirstKey"
      :vuex-second-key="tabs.electricMonitoringDetail.vuexSecondKey"
      :columns-std="tabs.electricMonitoringDetail.columnsStd"
      :table-config="tabs.electricMonitoringDetail.tableConfig"
...
<script>
  ...
  import {detail} from '@/api/environmentMonitor/electricMonitor'
  import moment from 'moment'

  export default {
    name: "electricMonitoringDetail",
    ...
    data() {
      ...
      return {
        detailData: '',
        tabKey: 'electricMonitoringDetail',
        buttonParentName: 'idc_electricMonitoringDetail', // 所有按钮对应的菜单名称（后台数据库存放的数据决定的）
        ...
        tableHeight: '300',
        pageHeight: 0,
        activeTab: 'electricMonitoringDetail',  // 当前激活的页签(对象的key值)
        ...
        // }],
        tabs: {
          electricMonitoringDetail: {
            label: '电量详细信息',
            type: 'normal',
            show: false,
            tableUniKey: 'id', // 作为表格数据唯一识别的字段名（即 属性 key）
            vuexFirstKey: 'electricMonitoringDetail', // 表格组件获取 vuex 数据的 一级Key
            vuexSecondKey: 'electricMonitoringDetail', // 表格组件获取 vuex 数据的 二级Key
        ...
        chartTitleShow: true,
        // chartTitleText: this.title + ' 用电量（kW·h）',
        chartTitleText: '用电量（kW·h）',
    ...
    methods: {
      ...
      setLineChartMin(chartData) {
        ...
        this.$refs['electricLineChart'].chartData.yAxisData.yAxisMin = min
      },
      ...
      setLineChartMax(chartData) {
        ...
        // console.log('max = ' + max)
        this.$refs['electricLineChart'].chartData.yAxisData.yAxisMax = max
      },

      setLineChartMinMax(chartData) {
        this.$refs['electricLineChart'].chartData.yAxisData.yAxisSplitNumber = 5
      ...
      toOneDayChartData(respData) {
        ...
        if (tmpData.length > 1) {
          for (let i = 1; i < tmpData.length; i++) {
            ...
            tmpData[i]['num'] = tmpData[i]['num'] / 100
      ...
      refreshTabTable(tableTag, curPage, curPageSize, callback) {
        ...
        // 表格的唯一标识和页签的唯一key相同
        switch (tableTag) {
          case 'electricMonitoringDetail': // 生产任务单
            // 请求<生产任务单>页签下的表格数据
      ...
      requestDetailData(curPage, curPageSize, callback) {
        ...
        //   "electricityNum": this.detailData['electricityNum'],
        ...
        this.tabs.electricMonitoringDetail.tableConfig.loading = true
        Promise.all(promiseArray).then(response => {
          ...
          if(electricMonitoringResponse.data.data.tableName!==undefined)
            electricMonitoringResponse.data.data.useList=electricMonitoringResponse.data.data.pointHisList
          // 接收页签的表格数据（后台数据）
          ...
          this.tabs.electricMonitoringDetail.tableConfig.loading = false
        })
      },
      ...
      receiveChartData(response) {
        ...
        if (response.code !== 200) {
          ...
          // this.tabs.electricMonitoringDetail.tableConfig.columnsStd = this.$store.state['electricMonitoringDetail'].moColumns.columnsStd
          // this.tabs.electricMonitoringDetail.tableConfig.columns = this.tableColumns
          // this.tabs.electricMonitoringDetail.tableConfig.countTotal = response.data.total
          // console.log('数据：' + JSON.stringify(tableData))
          // console.log(JSON.stringify(response.data))
          // 将后台数据转换成前端可用于显示折线统计图图形的数据
          let chartData = this.convertResponseToLineChartData(response.data)
          // console.log(JSON.stringify(chartData))
          this.$refs[`electricLineChart`].drawLine(chartData.xAxisData, chartData.seriesData)
        }
      },
      // 接收<生产任务单>页签的表格数据（后台数据）
      receiveMoTableData(response, callback) {
        ...
        if (response.code !== 200) {
          ...
          let tableData = [...response.data['useList']]
          ...
          tableData.map(item => {
            ...
            item['consumption'] = (!item['num']) ? 0 : item['num'] / 100
          ...
          // this.tabs.electricMonitoringDetail.tableConfig.columnsStd = this.$store.state['electricMonitoringDetail'].moColumns.columnsStd
          this.tabs.electricMonitoringDetail.tableConfig.columns = this.tableColumns
          this.tabs.electricMonitoringDetail.tableConfig.countTotal = 0 //response.data.total
          // console.log('数据：' + JSON.stringify(tableData))

          this.$refs[`tableEl_electricMonitoringDetail`].drawTableData(this.tabs.electricMonitoringDetail.tableConfig.columns, tableData)
          // 绘制生产工序的表格
          // console.log(JSON.stringify(response))
          ...
          if (callback) {
            ...
            this.$refs['tableEl_electricMonitoringDetail'].updateSelectedRowsToVuex(tableData, [])
          }
      ...
      requestElectricMonitoringDataById(data) {
        ...
        this.$refs[`electricLineChart`].drawLine([], [])
        this.$refs[`tableEl_electricMonitoringDetail`].drawTableData(this.tableColumns, [])

        // this.requestChartData()
        this.refreshTabTable('electricMonitoringDetail', this.tabs.electricMonitoringDetail.tableConfig.curPage, this.tabs.electricMonitoringDetail.tableConfig.pageSize, null)
      },
