<template>
  <div class="container">
    <add-card-batch ref="addCard" :cardData="cardData" :hasHeadSubTitle="cardConfig.hasHeadSubTitle"
                    ...
                    :serverPage="tabs.electronicUsageMonitor.serverPage"
                    :hasPageBar="true"
                    :cardIcon="iconElectronic"
                    :tableConfig="tabs.electronicUsageMonitor.tableConfig"
      ...
      <template slot="switchOptions">
        <table-el
          ...
          :serverPage="tabs.electronicUsageMonitor.serverPage"
          :table-uni-key="tabs.electronicUsageMonitor.tableUniKey"
          :vuex-first-key="tabs.electronicUsageMonitor.vuexFirstKey"
          :vuex-second-key="tabs.electronicUsageMonitor.vuexSecondKey"
          :columns-std="tabs.electronicUsageMonitor.columnsStd"
          :table-config="tabs.electronicUsageMonitor.tableConfig"
          :hasAddButton="tabs.electronicUsageMonitor.tableConfig.hasAddButton"
    ...
    <el-dialog
      top="50px" :fullscreen="isFullScreen"
      :visible.sync="dialogConfig.visible" left width="90%">
      <electric-monitoring-detail ref="detailComponent"
                                  :title="detailDialogTitle"
                                  style="margin-top: -40px"></electric-monitoring-detail>
...
<script>
  ...
  import {findElectricList} from "@/api/environmentMonitor/electricMonitor";
  import electricMonitoringDetail
    from "@/view/environmentMonitor/electronicUsageMonitor/components/electricMonitoringDetail.vue"
  ...
  export default {
    ...
    data() {
      return {
        ...
        tabs: {
          electronicUsageMonitor: {
            label: '用电检测',
            type: 'normal',
            show: true,
            tableUniKey: 'id', // 作为表格数据唯一识别的字段名（即 属性 key）
            vuexFirstKey: 'electronicUsageMonitor', // 表格组件获取 vuex 数据的 一级Key
            vuexSecondKey: 'electronicUsageMonitor', // 表格组件获取 vuex 数据的 二级Key
            serverPage: true, // 分页模式，true=后台分页，false=前台分页
            // 记录所有的数据的列字段，包括不用显示的列字段（有些字段无需显示，但需要与后台进行交互）
            columnsStd: [],
            tableConfig: {
              loading: false,
              tableName: '用电检测',  // 表格名称，在导出成Excel时，会使用该名称作为文件名
              columns: [],
    ...
    methods: {
      initData() {
        this.requestData(this.tabs.electronicUsageMonitor.tableConfig.curPage, this.tabs.electronicUsageMonitor.tableConfig.pageSize)
      },

      changeCurrentPage(curPage) {
        this.tabs.electronicUsageMonitor.tableConfig.curPage = curPage
        this.requestData(this.tabs.electronicUsageMonitor.tableConfig.curPage, this.tabs.electronicUsageMonitor.tableConfig.pageSize)
      },
      changPageSize(pageSize) {
        this.tabs.electronicUsageMonitor.tableConfig.pageSize = pageSize
        this.requestData(this.tabs.electronicUsageMonitor.tableConfig.curPage, this.tabs.electronicUsageMonitor.tableConfig.pageSize)
      },

      requestData(curPage, curPageSize, callback) {
        this.switchDisabled = true
        this.tabs.electronicUsageMonitor.tableConfig.loading = true
        ...
        let promiseArray = [
          findElectricList(param)
        ]

        this.appendRequestFilterParams('electronicUsageMonitor', param)
        ...
        Promise.all(promiseArray).then(response => {
          ...
          this.tabs.electronicUsageMonitor.tableConfig.loading = false
        })
      },
      //转换成卡片数据
      receiveCardData(response) {
        ...
        if (response.code !== 200) {
          ...
          this.tabs.electronicUsageMonitor.tableConfig.countTotal = response.data.total
      ...
      receiveTableData(response) {
        ...
        if (response.code !== 200) {
          ...
          tableData.map(item => {
            ...
            item['monitorName'] = `${item['collectNumber']}`;
            item['consumption'] = (!item['pointValue']) ? 0 : parseFloat(item['pointValue']).toFixed(1)
            item['consumptionTitle'] = '电量'
            item['consumptionUnit'] = `kW·h`
            item['voltage'] = 220 // todo: 电压缺字段
            item['electricity'] = ((!item['pointValue']) ? 0 : item['pointValue'] / 220).toFixed(2) // todo: 电流缺字段
          })

          // console.log(tableData)
          this.tabs.electronicUsageMonitor.tableConfig.columns = this.detailViewTableColumns
          this.tabs.electronicUsageMonitor.tableConfig.countTotal = response.data.total
          this.$refs[`tableEl_electronicUsageMonitor`].drawTableData(this.detailViewTableColumns, tableData)
      ...
      handSwitch(data) {
        ...
        this.$nextTick(() => {
          this.handleRequestTablePageData('electronicUsageMonitor',
            this.tabs.electronicUsageMonitor.tableConfig.curPage,
            this.tabs.electronicUsageMonitor.tableConfig.pageSize,
            null)
        })
      },
      //表格界面获取数据
      ...
      //查询，根据监测点名称
      handleSearch() {
        this.tabs.electronicUsageMonitor.tableConfig.curPage = 1;
        this.tabs.electronicUsageMonitor.tableConfig.pageSize = 1;
        this.switchDisabled = true;
        this.tabs.electronicUsageMonitor.tableConfig.loading = true;
        const param = {
          current_page: this.tabs.electronicUsageMonitor.tableConfig.curPage,
          page_size: this.tabs.electronicUsageMonitor.tableConfig.pageSize,
          orgId: this.$store.state.user.userInfo.orgId,
          containerName:this.tabs.electronicUsageMonitor.tableConfig.search
        }
        let promiseArray = [
          findElectricList(param)
        ]
