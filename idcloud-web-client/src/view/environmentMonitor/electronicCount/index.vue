<template>
  <div>
    ...
    <table-el
      ...
      :serverPage="tabs.electronicCount.serverPage"
      :table-uni-key="tabs.electronicCount.tableUniKey"
      :vuex-first-key="tabs.electronicCount.vuexFirstKey"
      :vuex-second-key="tabs.electronicCount.vuexSecondKey"
      :columns-std="tabs.electronicCount.columnsStd"
      :table-config="tabs.electronicCount.tableConfig"
      :hasAddButton="tabs.electronicCount.tableConfig.hasAddButton"
...
<script>
  ...
  import {usageStatistics} from "@/api/environmentMonitor/electricMonitor";
  ...
  export default {
      ...
      return {
        ...
        tabs: {
          electronicCount: {
            ...
            vuexFirstKey: 'electronicCount', // 表格组件获取 vuex 数据的 一级Key
            vuexSecondKey: 'electronicCount', // 表格组件获取 vuex 数据的 二级Key
    ...
    mounted(){
      this.requestData(this.tabs.electronicCount.tableConfig.curPage, this.tabs.electronicCount.tableConfig.pageSize);
    },
    methods: {
      //表格界面获取数据
      ...
      requestData(curPage, curPageSize, callback) {
        this.tabs.electronicCount.tableConfig.loading = true;
        const param = {
          orgId: this.$store.state.user.userInfo.orgId,
          type:this.type.vModel,
          time:this.type.vModel==='2'?undefined:this.queryTime,
        };
        if(this.tabs.electronicCount.tableConfig.search!=='')
          param[this.$refs[`tableEl_electronicCount`].searchTextContentConfig.value]=this.tabs.electronicCount.tableConfig.search;
        let promiseArray = [
        ...
        Promise.all(promiseArray).then(response => {
          ...
          this.tabs.electronicCount.tableConfig.loading = false;
        })
      },
      //接受到的数据处理
      receiveTableData(response) {
        ...
        if (response.code !== 200) {
          ...
          this.tabs.electronicCount.tableConfig.columns = this.detailViewTableColumns;
          this.tabs.electronicCount.tableConfig.countTotal = tableData.length;
          //前台分页处理（操作空间）
          // this.tabs.electronicCount.tableConfig.curPage
          // this.tabs.electronicCount.tableConfig.pageSize
          let autoData=tableData.slice(
            (this.tabs.electronicCount.tableConfig.curPage-1)*this.tabs.electronicCount.tableConfig.pageSize,
            this.tabs.electronicCount.tableConfig.curPage*this.tabs.electronicCount.tableConfig.pageSize
          );
          //
          this.$refs[`tableEl_electronicCount`].drawTableData(this.detailViewTableColumns, autoData);
        }
      },
