<template>
  <div class="container">
    <add-card-batch ref="addCard" :cardData="cardData" :hasHeadSubTitle="cardConfig.hasHeadSubTitle"
                    ...
                    :serverPage="tabs.temperatureMonitoring.serverPage"
                    :hasPageBar="true"
                    :cardIcon="iconTemperature"
                    :tableConfig="tabs.temperatureMonitoring.tableConfig"
      ...
      <template slot="switchOptions">
        <table-el
          ...
          :serverPage="tabs.temperatureMonitoring.serverPage"
          :table-uni-key="tabs.temperatureMonitoring.tableUniKey"
          :vuex-first-key="tabs.temperatureMonitoring.vuexFirstKey"
          :vuex-second-key="tabs.temperatureMonitoring.vuexSecondKey"
          :columns-std="tabs.temperatureMonitoring.columnsStd"
          :table-config="tabs.temperatureMonitoring.tableConfig"
          :hasAddButton="tabs.temperatureMonitoring.tableConfig.hasAddButton"
    ...
    <el-dialog
      ...
      <temperature-monitoring-detail ref="detailComponent"
                                     :title="detailDialogTitle"
                                     style="margin-top: -40px"></temperature-monitoring-detail>
...
<script>
  ...
  import {findTemperatureList} from "@/api/environmentMonitor/temperatureMonitor";
  import temperatureMonitoringDetail
    from "@/view/environmentMonitor/temperatureMonitoring/components/temperatureMonitoringDetail.vue"
  ...
  export default {
    ...
    components: {addCardBatch, TableEl, temperatureMonitoringDetail},
    data() {
      return {
        iconTemperature,
        tableHeight: '610',
        tabKey: 'temperatureMonitoring',
        cardData: [],
        ...
        tabs: {
          temperatureMonitoring: {
            ...
            vuexFirstKey: 'temperatureMonitoring', // 表格组件获取 vuex 数据的 一级Key
            vuexSecondKey: 'temperatureMonitoring', // 表格组件获取 vuex 数据的 二级Key
    ...
    methods: {
      initData() {
        ...
        this.requestData(this.tabs.temperatureMonitoring.tableConfig.curPage, this.tabs.temperatureMonitoring.tableConfig.pageSize)
      },
      changeCurrentPage(curPage){
        this.tabs.temperatureMonitoring.tableConfig.curPage=curPage;
        this.requestData(this.tabs.temperatureMonitoring.tableConfig.curPage, this.tabs.temperatureMonitoring.tableConfig.pageSize);
      },
      changPageSize(pageSize){
        this.tabs.temperatureMonitoring.tableConfig.pageSize=pageSize;
        this.requestData(this.tabs.temperatureMonitoring.tableConfig.curPage, this.tabs.temperatureMonitoring.tableConfig.pageSize);
      },

      requestData(curPage, curPageSize, callback) {
        this.switchDisabled = true;
        this.tabs.temperatureMonitoring.tableConfig.loading = true;
        const param = {
        ...
        let promiseArray = [
          findTemperatureList(param)
        ];
        this.appendRequestFilterParams('temperatureMonitoring', param);
        Promise.all(promiseArray).then(response => {
          ...
          this.tabs.temperatureMonitoring.tableConfig.loading = false;
        })
      },
      ...
      receiveCardData(response) {
        ...
        if (response.code !== 200) {
          ...
          this.tabs.temperatureMonitoring.tableConfig.countTotal = response.data.total
      ...
      receiveTableData(response) {
        ...
        if (response.code !== 200) {
          ...
          this.tabs.temperatureMonitoring.tableConfig.columns = this.detailViewTableColumns
          this.tabs.temperatureMonitoring.tableConfig.countTotal = response.data.total
          this.$refs[`tableEl_temperatureMonitoring`].drawTableData(this.detailViewTableColumns, tableData)
      ...
      handSwitch(data) {
        ...
        // this.initData()
        // this.handleRequestTablePageData('temperatureMonitoring', 1, 10, null)
        this.$nextTick(() => {
          this.handleRequestTablePageData('temperatureMonitoring',
            this.tabs.temperatureMonitoring.tableConfig.curPage,
            this.tabs.temperatureMonitoring.tableConfig.pageSize,
            null)
        })
      },
      //表格界面获取数据
      ...
      //查询，根据监测点名称
      handleSearch() {
        this.tabs.temperatureMonitoring.tableConfig.curPage=1;
        this.tabs.temperatureMonitoring.tableConfig.pageSize=10;
        this.switchDisabled = true;
        this.tabs.temperatureMonitoring.tableConfig.loading = true;
        const param = {
          current_page:  this.tabs.temperatureMonitoring.tableConfig.curPage,
          page_size: this.tabs.temperatureMonitoring.tableConfig.pageSize,
          orgId: this.$store.state.user.userInfo.orgId,
          containerName:this.tabs.temperatureMonitoring.tableConfig.search
        }
