<template>
  <div id="container"
    ...
    <table-el
      ...
      :table-tag="tabKey"
      :serverPage="tabs.temperatureMonitoringDetail.serverPage"
      :table-uni-key="tabs.temperatureMonitoringDetail.tableUniKey"
      :vuex-first-key="tabs.temperatureMonitoringDetail.vuexFirstKey"
      :vuex-second-key="tabs.temperatureMonitoringDetail.vuexSecondKey"
      :columns-std="tabs.temperatureMonitoringDetail.columnsStd"
      :table-config="tabs.temperatureMonitoringDetail.tableConfig"
...
<script>
  ...
  import {detail} from '@/api/environmentMonitor/temperatureMonitor'
  import moment from 'moment'

  export default {
    name: "temperatureMonitoringDetail",
    ...
    data() {
      ...
      return {
        detailData: '',
        tabKey: 'temperatureMonitoringDetail',
        buttonParentName: 'idc_temperatureMonitoringDetail', // 所有按钮对应的菜单名称（后台数据库存放的数据决定的）
        ...
        tableHeight: '300',
        pageHeight: 0,
        activeTab: 'temperatureMonitoringDetail',  // 当前激活的页签(对象的key值)
        ...
        // }],
        tabs: {
          temperatureMonitoringDetail: {
            label: '温度详细信息',
            type: 'normal',
            show: false,
            tableUniKey: 'id', // 作为表格数据唯一识别的字段名（即 属性 key）
            vuexFirstKey: 'temperatureMonitoringDetail', // 表格组件获取 vuex 数据的 一级Key
            vuexSecondKey: 'temperatureMonitoringDetail', // 表格组件获取 vuex 数据的 二级Key
        ...
        chartTitleShow: true,
        // chartTitleText: this.title + ' 温度信息（℃）',
        chartTitleText: '温度（℃）',
    ...
    methods: {
      ...
      convertResponseToLineChartData(respData) {
        const tmp = respData
        respData = respData['temperatureUseList']
      ...
      setLineChartMin(chartData) {
        ...
        this.$refs['temperatureLineChart'].chartData.yAxisData.yAxisMin = min
      },
      ...
      setLineChartMax(chartData) {
        ...
        // console.log('max = ' + max)
        this.$refs['temperatureLineChart'].chartData.yAxisData.yAxisMax = max
      },

      setLineChartMinMax(chartData) {
        this.$refs['temperatureLineChart'].chartData.yAxisData.yAxisSplitNumber = 5
      ...
      refreshTabTable(tableTag, curPage, curPageSize, callback) {
        ...
        // 表格的唯一标识和页签的唯一key相同
        switch (tableTag) {
          case 'temperatureMonitoringDetail':
      ...
      requestDetailData(curPage, curPageSize, callback) {
        ...
        //   "temperatureNum": this.detailData['temperatureNum'],
        ...
        this.tabs.temperatureMonitoringDetail.tableConfig.loading = true
        Promise.all(promiseArray).then(response => {
          ...
          if(electricMonitoringResponse.data.data.tableName!==undefined)
            electricMonitoringResponse.data.data.temperatureUseList=electricMonitoringResponse.data.data.pointHisList
          // 接收页签的表格数据（后台数据）
          ...
          this.tabs.temperatureMonitoringDetail.tableConfig.loading = false
        })
      },
      ...
      receiveChartData(response) {
        ...
        if (response.code !== 200) {
          ...
          this.$refs[`temperatureLineChart`].drawLine(chartData.xAxisData, chartData.seriesData)
        }
      },
      // 接收<生产任务单>页签的表格数据（后台数据）
      receiveMoTableData(response, callback) {
        ...
        if (response.code !== 200) {
          ...
          let tableData = [...response.data['temperatureUseList']]
          ...
          // this.tabs.temperatureMonitoringDetail.tableConfig.columnsStd = this.$store.state['temperatureMonitoringDetail'].moColumns.columnsStd
          this.tabs.temperatureMonitoringDetail.tableConfig.columns = this.tableColumns
          this.tabs.temperatureMonitoringDetail.tableConfig.countTotal = 0 //response.data.total
          // console.log('数据：' + JSON.stringify(tableData))

          this.$refs[`tableEl_temperatureMonitoringDetail`].drawTableData(this.tabs.temperatureMonitoringDetail.tableConfig.columns, tableData)
          // 绘制生产工序的表格
          // console.log(JSON.stringify(response))
          ...
          if (callback) {
            ...
            this.$refs['tableEl_temperatureMonitoringDetail'].updateSelectedRowsToVuex(tableData, [])
          }
      ...
      requestElectricMonitoringDataById(data) {
        ...
        this.$refs[`temperatureLineChart`].drawLine([], [])
        this.$refs[`tableEl_temperatureMonitoringDetail`].drawTableData(this.tableColumns, [])

        // this.requestChartData()
        this.refreshTabTable('temperatureMonitoringDetail', this.tabs.temperatureMonitoringDetail.tableConfig.curPage, this.tabs.temperatureMonitoringDetail.tableConfig.pageSize, null)
      },
