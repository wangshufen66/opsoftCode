<template>
  <div id="container"
    ...
    <table-el
      ...
      :table-tag="tabKey"
      :serverPage="tabs.gasMonitoringDetail.serverPage"
      :table-uni-key="tabs.gasMonitoringDetail.tableUniKey"
      :vuex-first-key="tabs.gasMonitoringDetail.vuexFirstKey"
      :vuex-second-key="tabs.gasMonitoringDetail.vuexSecondKey"
      :columns-std="tabs.gasMonitoringDetail.columnsStd"
      :table-config="tabs.gasMonitoringDetail.tableConfig"
...
<script>
  ...
  import {detail} from '@/api/environmentMonitor/gasMonitor'
  import moment from 'moment'

  export default {
    name: "gasMonitoringDetail",
    ...
    data() {
      ...
      return {
        detailData: '',
        tabKey: 'gasMonitoringDetail',
        buttonParentName: 'idc_gasMonitoringDetail', // 所有按钮对应的菜单名称（后台数据库存放的数据决定的）
        ...
        tableHeight: '300',
        pageHeight: 0,
        activeTab: 'gasMonitoringDetail',  // 当前激活的页签(对象的key值)
        ...
        tabs: {
          gasMonitoringDetail: {
            ...
            vuexFirstKey: 'gasMonitoringDetail', // 表格组件获取 vuex 数据的 一级Key
            vuexSecondKey: 'gasMonitoringDetail', // 表格组件获取 vuex 数据的 二级Key
    ...
    methods: {
      ...
      setLineChartMin(chartData) {
        ...
        this.$refs['gasLineChart'].chartData.yAxisData.yAxisMin = min
      },
      ...
      setLineChartMax(chartData) {
        ...
        // console.log('max = ' + max)
        this.$refs['gasLineChart'].chartData.yAxisData.yAxisMax =  max
      },

      setLineChartMinMax(chartData) {
        this.$refs['gasLineChart'].chartData.yAxisData.yAxisSplitNumber = 5
      ...
      requestDetailData(curPage, curPageSize, callback) {
        ...
        this.tabs.gasMonitoringDetail.tableConfig.loading = true
        Promise.all(promiseArray).then(response => {
          ...
          if(electricMonitoringResponse.data.data.tableName!==undefined)
            electricMonitoringResponse.data.data.gasUseList=electricMonitoringResponse.data.data.pointHisList
          // 接收页签的表格数据（后台数据）
          ...
          this.tabs.gasMonitoringDetail.tableConfig.loading = false;
        })
      },
      ...
      receiveChartData(response) {
        ...
        if (response.code !== 200) {
          ...
          //
          this.$refs[`gasLineChart`].drawLine(chartData.xAxisData, chartData.seriesData)
        }
      },
      // 接收<生产任务单>页签的表格数据（后台数据）
      receiveMoTableData(response, callback) {
        ...
        if (response.code !== 200) {
          ...
          let tableData = [...response.data['gasUseList']]
          ...
          tableData.map(item=>{
            item['monitorType'] = item['onlineStatus'] || 0 // 后台0=离线，1=在线，前端1=离线，2=在线，所以要转换一下
            item['monitorNumber']=item['attrHidden']
            item['monitorNo'] = item['attrHidden']
            item['monitorName']=item['attrName']
            item['consumption']=(!item['num'])?0:item['num']
            item['consumptionTitle'] = '燃气量'
          ...
          // this.tabs.temperatureMonitoringDetail.tableConfig.columnsStd = this.$store.state['temperatureMonitoringDetail'].moColumns.columnsStd
          this.tabs.gasMonitoringDetail.tableConfig.columns = this.tableColumns
          this.tabs.gasMonitoringDetail.tableConfig.countTotal = 0 //response.data.total
          // console.log('数据：' + JSON.stringify(tableData))

          this.$refs[`tableEl_gasMonitoringDetail`].drawTableData(this.tabs.gasMonitoringDetail.tableConfig.columns, tableData)
          // 绘制生产工序的表格
          // console.log(JSON.stringify(response))
          ...
          if (callback) {
            ...
            this.$refs['tableEl_gasMonitoringDetail'].updateSelectedRowsToVuex(tableData, [])
          }
      ...
      requestElectricMonitoringDataById(data) {
        ...
        this.$refs[`gasLineChart`].drawLine([], [])
        this.$refs[`tableEl_gasMonitoringDetail`].drawTableData(this.tableColumns, [])

        // this.requestChartData()
        this.refreshTabTable('gasMonitoringDetail', this.tabs.gasMonitoringDetail.tableConfig.curPage, this.tabs.gasMonitoringDetail.tableConfig.pageSize, null)
      },
