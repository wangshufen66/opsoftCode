  ...
  methods: {
    ...
    async addOrUpdateForm() {
      ...
      if (
        ...
        this.tableDetailForm.idForm.data.pointList &&
        this.tableDetailForm.idForm.data.pointList.length > 0
      ) {
        this.tableDetailForm.idForm.data.pointList =
          this.tableDetailForm.idForm.data.pointList.map((item) => {
            switch (item.style) {
              case '默认':
                item.style = 1;
                break;
      ...
      console.log('tijiao', this.tableDetailForm.idForm.data);
    ...
    // 拷贝form
    copyTableForm() {
      console.log('43242', this.tableDetailForm.idForm.data);
      //这写的不合理（需要改进）
      this.$emit('initTableDetail', {
        ...
        name: '',
        createTime: '', //创建时间
        box8303BrandId: this.tableDetailForm.idForm.data.box8303BrandId,
        box8303BrandName: this.tableDetailForm.idForm.data.box8303BrandName,
        updateTime: '', //最后更新时间
      });
      this.goTableForm();
    },
            ...
            if (this.formList.length > 0) {
              this.formList.forEach((fItem, index) => {
                ...
                if (res.data.data.hasOwnProperty(fItem.fromList)) {
                  res.data.data[fItem.fromList].map((value) => {
                    switch (value.style) {
                      case 1:
                        value.style = '默认';
                        break;
                  ...
                  console.log(this.listElTabs.pages[index].tableData);
    ...
    async listFormConfirm(ref, index) {
      this.$refs[ref][0].$refs['idForm'].validate(async (valid) => {
        if (valid) {
          ...
          // console.log(this.formList[index].form.data);
