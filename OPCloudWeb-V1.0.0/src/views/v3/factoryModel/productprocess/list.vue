...
<template>
  ...
  <div class="m-page-list">
    <div class="m-page-list-search">
      <el-form mode="topForm" class="topForm" :inline="true" @submit.prevent>
        ...
        <div class="top-serach">
          <el-form-item label="工艺流程编码">
            <el-input
              type="text"
              v-model.trim="searchForm.processRouteNbr"
          ...
          <el-form-item label="工艺流程名称">
            <el-input
              type="text"
              v-model.trim="searchForm.processRouteName"
              @keyup.enter="tableGatherFunc.reLoadTableData"
              clearable
            ></el-input>
          </el-form-item>
    ...
    <div class="m-page-list-table" style="height:100%">
      ...
      >
        <template #col_isDisabled="{ data }">{{ renderDictionTag(data.isDisabled, 'isDisabled') }}</template>
  ...
  <div class="fixedDialog">
    <el-dialog v-model="editDialog.visible" custom-class="dialog_body_padding_5" :destroy-on-close="true" width="1400px">
      <template #title>
         <dialog-title :title="editDialog.param.showType == 1?'新增工序':editDialog.param.showType == 2?'编辑工序':'查看工序'" ></dialog-title>
      </template>
