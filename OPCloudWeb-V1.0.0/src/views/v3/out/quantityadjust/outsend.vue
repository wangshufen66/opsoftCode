    ...
    <div class="productin-start">
        ...
        <el-dialog v-model="dialogVisible" @close="dialoagClose" destroy-on-close width="600px">
            <template v-slot:title>
                <dialog-title title="委外发出数量调整"></dialog-title>
            </template>
            <el-form :model="dataForm" ref="formRef" :rules="formRules" label-width="130px">
                <el-form-item label="委外发出单号">
                    <el-input v-model="dataForm.billNo" maxlength="32" clearable disabled></el-input>
                </el-form-item>
                <el-form-item label="原委外发出数量">
                    <el-input disabled v-model="dataForm.outQty" maxlength="32" clearable></el-input>
                </el-form-item>
                <el-form-item label="新委外发出数量" prop="newQuantity">
                ...
                <el-form-item label="调整数量">
                    <el-input v-model="dataForm.adjustQuantity" clearable></el-input>
                </el-form-item>
...
import tableColums from '@/config/tableConfig/adjustRecord/outSend';
import { getStorage, messageAlert } from '@/utils/utils';
import { adjustOutsourceOut, getOutsourceOut } from "@/api/homeApi/index"
...
watch(() => dataForm.newQuantity, (newVal, oldVal) => {
    if (isNaN(parseInt(dataForm.newQuantity))) {
        ...
        dataForm.adjustQuantity = Number(dataForm.newQuantity) - Number(dataForm.outQty)
    }
})
...
async function getDataList() {
    ...
    const res = await getOutsourceOut(params);
    const { code, data } = res;
    if (code === 200) {
...
function onSubmitForm() {
    formRef.value.validate(async (valid: boolean) => {
        ...
        const res = await adjustOutsourceOut(params)
        if (res.code == 200) {
