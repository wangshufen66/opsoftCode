    ...
    <div class="productin-start">
          <el-form :inline="true" class="top-search">
               <el-form-item label="生产订单">
                <el-input clearable v-model.trim="searchForm.orderNbr" @keyup.enter="onSearch"></el-input>
            </el-form-item>
        ...
        >
            <template #col_fstartTime="{ data }">
                <span>{{ formatfTime(data.fstartTime) }}</span>
            </template>
            <template #col_freportTime="{ data }">
                <span>{{ formatfTime(data.freportTime) }}</span>
            </template>
        ...
        <el-dialog v-model="dialogVisible" @close="dialogClosed" destroy-on-close width="600px">
            <template v-slot:title>
                <dialog-title title="汇报数量调整"></dialog-title>
            </template>
            <el-form :model="dataForm" ref="formRef" :rules="formRules" label-width="120px">
                <el-form-item label="汇报单号">
                    <el-input v-model="dataForm.freportNo" maxlength="32" clearable disabled></el-input>
                </el-form-item>
                <el-form-item label="原数量">
                    <el-input v-model="dataForm.reportQty" maxlength="32" clearable disabled></el-input>
                </el-form-item>
                <el-form-item label="新汇报数量" prop="newQuantity">
...
import tableColums from '@/config/tableConfig/adjustRecord/productionreport';
import { getStorage, messageAlert } from '@/utils/utils';

import { getReportOrderList, adjustReportOrder } from "@/api/homeApi/index"
...
const formRules = reactive({
    newQuantity: [{
        required: true, message: '请输入的新的汇报数量',
    },
    { type: 'number', message: '输入的汇报数量必须为数字形式' }
    ],
...
watch(() => dataForm.newQuantity, (newVal, oldVal) => {
    if (isNaN(parseInt(dataForm.newQuantity))) {
        ...
        dataForm.adjustQuantity = Number(dataForm.newQuantity) - Number(dataForm.reportQty)
    }
})
...
async function getDataList() {
    ...
    const res = await getReportOrderList(params);
    const { code, data } = res;
    if (code === 200) {
...
async function onAdjust(row: any) {
    ...
    nextTick(() => {
        dataForm.freportNo = row.freportNo
        dataForm.reportQty = row.reportQty
        dataForm.newQuantity = row.newQuantity
        dataForm.id = row.id
...
async function onSubmitForm() {
    formRef.value.validate(async (valid: boolean) => {
        ...
        const res = await adjustReportOrder(params);
        const { code } = res;
        if (code == 200) {
