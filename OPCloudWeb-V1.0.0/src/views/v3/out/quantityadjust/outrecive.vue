    ...
    <div class="productin-start">
        <el-form :inline="true" class="top-search">
            <el-form-item label="生产订单">
                <el-input  clearable v-model.trim="searchForm.icmoNumber"></el-input>
            </el-form-item>
            <el-form-item label="产品名称">
                <el-input  clearable v-model.trim="searchForm.productName"></el-input>
            </el-form-item>
        ...
        <el-dialog v-model="dialogVisible" @close="dialogClosed" destroy-on-close width="600px">
            <template v-slot:title>
                <dialog-title title="委外接收数量调整"></dialog-title>
            </template>
            <el-form :model="dataForm" ref="formRef" :rules="formRules" label-width="130px">
                <el-form-item label="委外接收单号">
                    <el-input maxlength="32" v-model="dataForm.billNo" clearable disabled></el-input>
                </el-form-item>
                <el-form-item label="原委外接收数量">
                    <el-input maxlength="32" v-model="dataForm.rcvQty" clearable disabled></el-input>
                </el-form-item>
                <el-form-item label="新委外接收数量" prop="newQuantity">
                ...
                <el-form-item label="调整数量">
                    <el-input v-model="dataForm.adjustQuantity" disabled clearable></el-input>
                </el-form-item>
                <el-form-item label="调整人">
                    <el-input v-model="nickName" disabled clearable></el-input>
                </el-form-item>
                <el-form-item label="调整时间">
                    <el-input v-model="dataForm.currentTime" disabled  clearable></el-input>
                </el-form-item>
            </el-form>
            <template #footer>
...
import tableColums from '@/config/tableConfig/adjustRecord/outrecive';
import { getStorage, messageAlert } from '@/utils/utils';
import { getOutsourceReceive, adjustOutsourceReceive } from '@/api/homeApi/index'
...
watch(() => dataForm.newQuantity, (newVal, oldVal) => {
    if (isNaN(parseInt(dataForm.newQuantity))) {
        ...
        dataForm.adjustQuantity = Number(dataForm.newQuantity) - Number(dataForm.reciveOty)
    }
})
...
async function getDataList() {
    ...
    const res = await getOutsourceReceive(params);
    const { code, data } = res;
    if (code === 200) {
...
async function onAdjust(row: any) {
    ...
    nextTick(() => {
        dataForm.id = row.id
        dataForm.billNo=row.billNo
        dataForm.rcvQty=row.rcvQty
...
function onSubmitForm() {
    formRef.value.validate(async (valid: boolean) => {
        ...
        const res = await adjustOutsourceReceive(params)
        if (res.code == 200) {
