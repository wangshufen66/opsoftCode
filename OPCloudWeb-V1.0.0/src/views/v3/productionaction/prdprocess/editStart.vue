      ...
      <rotate-row title="任务信息">
        ...
        <el-col :span="8">
          <el-form-item label="可开工数量" prop="couldStartQty">
            <el-input
              v-model="pageData.formData.couldStartQty"
              @click.stop="openSelectDialogFunc.show"
              disabled
            ></el-input>
          </el-form-item>
...
const { dialogFacotry } = useSelectList(pageData)
...
const formRules = reactive({
  ...
  startQty: [{ required: true, message: "该字段必填", trigger: "change" }],
...
const getFormData = async () => {
  if (props.param.id.length > 0) {
    ...
    pageData.formData.processId = pageData.formData.id
    pageData.formData.startTime = new Date()
    delete (pageData.formData.id)
  }
};
...
const openSelectDialogFunc = OpenSelectDialogFunc(
  openSelectDialog,
  openSelectRef
);
  ...
  return new Promise((resolve, reject) => {
    addPageRef.value.validate(async (valid: boolean) => {
      if (!valid) { return; }
        ...
        var res = await prdStartApi.SaveOrUpdate(postParam);
        if (res.code == 200) {
