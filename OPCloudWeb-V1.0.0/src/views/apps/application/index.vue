...
import useProps from "@/components/commontable/useProps";
...
async function initPageInfo() {
  ...
  const menus = menuLayout.value.menus;
  const currentApp = menus.find((f) => f.key === appid);
  if (!currentApp || !currentApp.children) return;
  currentPage.appName = currentApp.title;
  const currentModel = currentApp.children.find(
    (f) => f.key === `${appid}/${modelid}`
  );
  if (!currentModel || !currentModel.children) return;
  const page = currentModel.children.find(
    (f) => f.key === `${appid}/${modelid}/${pageid}`
  );
  if (!page) return;
  pageParmas.modelName = currentModel.title;
  pageParmas.modelId = modelid;
  pageParmas.pageId = pageid as string;
  pageParmas.pageName = page.title;

  const res = await getPageInfo({ pageId: pageid });
  if (res.code == 200) {
...
async function onSubmitTableForm(
  ...
  transformDataSourceOfTableData(tableForm.componentList, [row]).then(() => {
    tableForm.formType === 1 && dataSource.push(row);
  });
