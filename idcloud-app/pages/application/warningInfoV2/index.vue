<template>
	<view class="content">
		...
		<view style="margin-left: 32rpx; margin-top: 12rpx;" class="title-style">预警次数</view>
		<canvas v-show="hasFirst" canvas-id="canvasColumn" id="canvasColumn" class="charts" :width="cWidth*pixelRatio" :height="cHeight*pixelRatio"
		...
		<view style="padding: 0 32rpx 32rpx 32rpx">
			...
			<u-table font-size="22">
				...
				<u-tr v-for="(item,index) in tableData" :key="index">
					<u-td>{{item.deviceNumber}}</u-td>
					<u-td>{{item.deviceName}}</u-td>
					<u-td v-show="activeColumn == 0">{{item.day}}</u-td>
					<u-td v-show="activeColumn == 1">{{item.week}}</u-td>
					<u-td v-show="activeColumn == 2">{{item.month}}</u-td>
				</u-tr>
			</u-table>
		</view>
		...
		methods: {
			...
			async getColumnData() {
				try {
					...
					this.hasFirst = true
					let res = await Promise.resolve(getErrorListV2(param))
					let categories = []
					let series = [{
						"name": "最近24小时",
						"data": []
