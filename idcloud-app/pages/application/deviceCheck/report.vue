	...
	<view class="report">
		...
		<view class="line-wrapper" style="padding-right: 2%;">
			...
			<view class="line-child">
				<u-radio-group width="160rpx" v-model="validate.checkResult" @change="handleCheckResultGroupChange">
					<u-radio v-for="(item, index) in checkResultList" :key="index" :name="item.key" :active-color="item.color">
						{{item.value}}
					</u-radio>
				</u-radio-group>
			</view>
				...
				try {
					...
					if (res[1].data.code === 200) {
						await this.$store.dispatch('setCheckResultStatus', {
							status: this.validate.checkResult,
