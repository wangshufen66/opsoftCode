<template>
	<view class="device-check">
		<view class="search-wrapper" style="padding-left: ;">
		...
		<view class="card-wrapper">
			...
			<view @click="handleClickCard(item)" v-for="(item,index) in dataList" :key="index" class="card-item">
				<view class="icon-wrapper">
					<image src="../../../static/ticketReport/dianjian.png" style="width: 44px;height: 44px;"></image>
				</view>
	...
	export default {
		...
		methods: {
			...
			handleClickSearch(val) {
				...
				this.dataList = this.originList.filter(i => {
					if(i[this.search.select]){
						return i[this.search.select].toLowerCase().indexOf(val.toLowerCase()) !== -1
					}
				})
			...
			handleClickCard(item) {
				...
				this.page.current = 1
				uni.navigateTo({
					url: "/pages/application/deviceCheck/ticket?jump=0"
				});
			},
