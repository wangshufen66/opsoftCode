<template>
	<view class="report" ref="detailContainer" :style="containerStyle">
		...
		<view class="line-wrapper">
			...
			<view v-if="detailData.checkResult == 0" style="color: green;" class="line-child">
				正常
			</view>
			<view v-else-if="detailData.checkResult == 1" style="color: red;" class="line-child">
				异常
			</view>
			...
			async handleDelete() {
				...
				try {
					...
					if (res[1].data.code == 200) {
						...
						setTimeout(() => {
							uni.navigateBack({
								delta: this.$store.state.device.isFromFinish ? 3 : 0
							})
						}, 1500);
					} else {
						uni.showToast({
