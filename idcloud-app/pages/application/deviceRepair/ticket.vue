	...
	methods: {
		...
		async getRecord() {
			...
			try {
				...
				let res = await Promise.resolve(getRecordList(param));
				this.loading = false;
				this.recordList = [];
		...
		async getSparePart() {
			...
			try {
				...
				let res = await Promise.resolve(getSparePartList(param));
				this.loading = false;
				this.sparePartList = [];
		...
		handleAddNewReport() {
			uni.navigateTo({
				url: '/pages/application/deviceRepair/report'
			});
		},
		async handleClickCard(item) {
			await this.$store.dispatch('setReportDetailInfo', item);
			uni.navigateTo({
				url: '/pages/application/deviceRepair/detail'
			});
		}
	}
};
