<template>
	<view class="content">
		...
		<view style="margin:18rpx 32rpx; display: flex; justify-content: space-between; line-height: 70rpx;" class="title-style">产量统计
		...
		<view style="margin-left: 32rpx; margin-top: 12rpx;" class="title-style">产量数据分析</view>
		...
		<view style="padding: 0 32rpx 32rpx 32rpx">
			...
			<u-table font-size="22">
				...
				<u-tr v-for="(item,index) in tableData" :key="index">
					<u-td>{{item.containerNumber}}</u-td>
					<u-td>{{item.containerName}}</u-td>
					<u-td v-show="activeColumnX == 0">{{item.sum}}</u-td>
					<u-td v-show="activeColumnX == 1">{{item.sum}}</u-td>
					<u-td v-show="activeColumnX == 2">{{item.sum}}</u-td>
				</u-tr>
			</u-table>
		</view>
	...
	export default {
		...
		methods: {
			...
			handlePickerConfirm(val) {
				...
				if (val[0].value == -1) {} else {
					if (val[0].value == val[1].value) {
						this.searchParam = {
							containerId: val[0].value
						}
					} else {
						this.searchParam = {
							containerId: val[1].value
						}
			...
			async getColumnData() {
				...
				let categories = []
				let series = [{
					name: this.selectList[this.activeColumn].name,
					data: []
				}]
				if (res[1].data.code == 200) {
			...
			async getColumnData2() {
				...
				if (res[1].data.code == 200) {
					...
					res[1].data.data.forEach(i => {
						...
						i.produceDateNumList[1] ? series[1].data.push(i.produceDateNumList[1].num) : series[1].data.push(0)
					})
				}
			...
			async getTree() {
				try {
					...
					this.searchParam = {
						containerId: this.selectorObj[0].value
					}
					this.getColumnData()
					this.getColumnData2()
					this.getTableData()
