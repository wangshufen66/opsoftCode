<template>
	<view class="shift_choose">
		<view class="search">
			...
			<u-search
				style="height: 70rpx;width: 65%;border: 1px solid #ccc;border-radius: 10rpx;"
				bg-color="#fff"
				search-icon=""
				:animation="true"
				shape="square"
		...
		<view class="shift_box">
			<view class="item" v-for="(item, i) in shiftList" :key="i">
				...
				<view class="right"><button type="primary" size="mini" :plain="true" @click="choseShift(item.name, item.id)">选择</button></view>
			</view>
...
<script>
import { getList } from '@/api/deviceProcess.js';
export default {
	data() {
		return {
			shiftList: [], // 班次列表
	...
	methods: {
		...
		choseShift(name, id) {
			...
			this.$globalEvent.emit('newdevice-select', { type: 'shift', json });
			// if(beforePage !=undefined){
			// 	if(beforePage.shift){//判断是否为小程序，小程序的数据都在data里
			// 		beforePage.shift = json;
			// 	}else{
			// 		beforePage.data.shift = json;
			// 	}
			// }
			uni.navigateBack();
		}
