	...
	<view class="shift_choose">
		...
		<view class="shift_box">
			<view class="item" v-for="(item,i) in shiftList" :key="i" >
				...
				<view class="right">
					<button type="primary" size="mini" :plain="true" @click="choseShift(item.name,item.id)">选择</button>
				</view>
			</view>
			<view class="empty" >
				{{empty}}
		...
		methods: {
			...
			async getShiftList() {
				try {
					...
					uni.showLoading()
					this.empty = '正在加载中'
					let res = await Promise.resolve(getStaffList(param))
					uni.hideLoading()
					this.empty = '没有更多了'
			...
			choseShift(name,id){
				...
				this.$globalEvent.emit('newdevice-select', { type: 'staff', json });
				// if(beforePage !=undefined){
				// 	if(beforePage.staff){//判断是否为小程序，小程序的数据都在data里
				// 		beforePage.staff = json;
				// 	}else{
				// 		beforePage.data.staff = json;
				// 	}
				// }
				uni.navigateBack();
			}
