<template>
	<view>
		<uni-nav-bar id="nav-bar" ref="navBar" :status-bar="true" title="设备地图" @clickLeft="Back">
			...
			<view slot="right" class="position">
				<picker mode="selector" @change="bindPickerChange" :value="index" :range="array">
					<view style="font-size: 16px; padding: 0;" class="uni-input">{{ array[index] }}</view>
				</picker>
			</view>
		</uni-nav-bar>
		<view :style="{ height: mapHeight }">
			<map id="deviceMap" ref="deviceMap" :latitude="latitude" :include-points="map.center" :longitude="longitude" :style="{ height: mapHeight }"
			 :markers="covers" :scale="map.scale" @markertap="clickMarker"></map>
		</view>
	</view>
</template>
