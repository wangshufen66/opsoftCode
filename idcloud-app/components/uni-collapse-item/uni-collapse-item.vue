<template>
	<view :class="{ 'uni-collapse-cell--disabled': disabled,'uni-collapse-cell--notdisabled': !disabled, 'uni-collapse-cell--open': isOpen,'uni-collapse-cell--hide':!isOpen }" class="uni-collapse-cell">
		<view class="uni-collapse-cell__title" :class="{'uni-collapse-cell__title-open': isOpen}" @click="onClick">
			<image v-if="thumb" :src="thumb" class="uni-collapse-cell__title-img" />
			<text class="uni-collapse-cell__title-text">{{ title }}</text>
			
			<!-- #ifdef MP-ALIPAY -->
			<view v-if="isOpen" :class="{ 'uni-collapse-cell--animation': showAnimation === true }" class="uni-collapse-cell__title-arrow">
				<text>{{rightTitle}}</text>
				<fa-icon color="#fff" size="20" type="caret-up" />
			</view>
			<view  v-else :class="{'uni-collapse-cell--animation': showAnimation === true }" class="uni-collapse-cell__title-arrow">
				<text>{{rightTitle}}</text>
				<fa-icon color="#fff" size="20" type="caret-down" />
			</view>
			<!-- #endif -->
			<!-- #ifndef MP-ALIPAY -->
			<view class="uni-collapse-cell-icon" :class="{'uni-collapse-cell--animation': showAnimation === true }" >
				<text>{{rightTitle}}</text>
				<fa-icon v-if="isOpen"  color="#fff" size="18" type="caret-up"></fa-icon>
				<fa-icon v-else   color="#fff" size="18" type="caret-down" />
			</view>
			<!-- #endif -->
		</view>
