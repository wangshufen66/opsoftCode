<!-- 表单属性配置 — 按钮配置 -->
<template>
  <div class="divider-wrap">表单按钮</div>
        ...
        <el-tab-pane label="行为" name="behavior">
          ...
          <el-form-item v-if="ele.fieldValueChange" :label-width="0">
            <div class="item-grid" v-for="(fitem, index) in ele.fieldValueChangeList" :key="index">
              ...
              >
                <el-option
                  v-for="(item,i) in masterComponent"
                  :key="i"
                  :value="item.value"
                  :label="item.label"
                ></el-option>
          ...
          <el-form-item label="显示方式" label-width="80px">
            <el-select v-model="ele.showCondition" placeholder="请选择显示方式" @change="showConditionChange(ele)">
          ...
          <el-form-item :label-width="1" v-if="ele.showCondition == 'byCondition'">
            <div v-for="(fitem, index) in ele.conditionList" :key="index">
              <div>
                <el-select v-model="fitem.conditionName" placeholder="主表字段" style="width:100px;margin-left:10px;">
                  <el-option
                    v-for="(item,i) in masterComponent"
                    :key="i"
                    :label="item.label"
                ...
                <el-select v-model="fitem.conditionCompare" placeholder="比较" style="width:80px;margin-left:5px;">
                  <el-option
                ...
                <el-input v-model="fitem.conditionValue" placeholder="值" clearable style="width:70px;margin-left:5px;"></el-input>
                <el-button icon="el-icon-circle-plus-outline" type="text" @click="addConditionFiled(i)" style="margin-left:3px;" />
                <el-button
                  icon="el-icon-remove-outline"
                  v-if="ele.conditionList.length > 1"
                  type="text"
                  @click="removeConditionFiled(i, index)"
                  style="margin-left:3px;"
                />
          ...
          <el-form-item v-if="ele.copyPageId" :label-width="1">
            <div class="item-grid" v-for="(item, index) in ele.dataCopyFieldList" :key="index">
              ...
              <div class="item-oper-wrap">
                <el-select v-model="item.targetFieldId" filterable clearable placeholder="目标字段">
                  <el-option
                    v-for="(item,i) in tableFieldList[i]"
                    :key="i"
...
export default {
  ...
  setup(props) {
    ...
    const addConditionFiled = (i) => {
      if (props.formConf?.buttonList[i].conditionList) {
        props.formConf?.buttonList[i].conditionList.push(
          {
            conditionName: "",
            conditionCompare: "",
            conditionValue: "",
    ...
    const removeConditionFiled = (i, index) => {
      if (props.formConf?.buttonList[i].conditionList) {
        props.formConf?.buttonList[i].conditionList.splice(index, 1);
      }
    };
    ...
    async function initConditionList() {
      if (props.formConf?.buttonList && props.formConf?.buttonList.length > 0) {
        for (var i = 0; i < props.formConf?.buttonList.length; i++) {
          if (!props.formConf?.buttonList[i].showCondition) {
            return;
          }
          if (props.formConf?.buttonList[i].showCondition == 'byCondition') {
            if(!props.formConf?.buttonList[i].conditionList){
              props.formConf.buttonList[i].conditionList = [];
              props.formConf.buttonList[i].conditionList.push({
                conditionName: "",
                conditionCompare: "",
                conditionValue: "",
    ...
    function showConditionChange(ele){
      if(ele.showCondition == "byCondition"){
        if(!ele.conditionList){
          ele.conditionList = [];
          ele.conditionList.push({
            conditionName: "",
            conditionCompare: "",
            conditionValue: "",
          });
