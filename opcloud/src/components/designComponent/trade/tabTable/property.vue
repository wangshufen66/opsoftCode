...
<template>
  <el-form :model="formModel" :rules="formRules" ref="formRef" label-width="100px" size="small">
    ...
    <div class="property-additem">
      <el-button class="btn-operate" icon="el-icon-circle-plus-outline" type="text" @click="addTabItem">添加选项</el-button>
    </div>
    <template v-if="componentCfg?.tabOptions && componentCfg.tabOptions.length > 0">
      ...
      <el-form-item label="主表字段">
        <el-select v-model="relFields.masterField" filterable clearable placeholder="默认主表ID字段" @change="onMasterFieldChange">
      ...
      <el-form-item label-width="138px">
        ...
        <el-radio-group v-model="relFields.isDeleteSlave">
...
import columnSetting from "@/components/core/tableExtend/columnSetting.vue";
import draggable from "vuedraggable";
import { ITableColunmSetting } from "@/shared/tableConfig";
import { TabTableComponent, RelField } from "./IConfig";
import { useDesign } from "@/hooks/use-design";

export default {
  name: "TabTableProperty",
  ...
  setup(props) {
    ...
    const relFields = ref<RelField>({
      pid: undefined,
      masterField: undefined,
      field: undefined,
      isDeleteSlave: undefined,
    });
    ...
    function onMasterFieldChange(e) {
      relFields.value.masterField =
        relFields.value.masterField === "" ? null : relFields.value.masterField;
    }
