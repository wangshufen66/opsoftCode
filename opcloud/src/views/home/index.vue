...
async function getHomePageConfig() {
  ...
  if (res.code == 200) {
    const data = res.data
    reportConfig.url = data?.url;
    reportConfig.token = data?.token;
  }
}
getHomePageConfig()
...
// 跳转模块编辑页
const openAppPage = (item) => {
  /**菜单结构数据 */
  const menuLayout: any = store.state.comm.menuLayout;
  const app = menuLayout.menus.find(f => f.key === item.menuId);
  ...
  if (layoutState) {
    const has = layoutState.pageTabList.value.findIndex(f => f.name === page.path) > -1;
    if (!has) {
