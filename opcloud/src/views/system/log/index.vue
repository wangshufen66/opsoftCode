        ...
        <div class="search-wrap">
            <el-form class="form-wrap" :model="searchForm" :inline="true" @submit.prevent>
                ...
                <el-form-item label="操作人">
                    <el-select v-model="searchForm.userId" placeholder="请选择操作人">
                        <el-option
                            v-for="item in users"
                            :key="item.key"
                            :label="item.value"
            ...
            <template #operatColumn>
                <el-table-column fixed="right" label="操作" min-width="300">
                    <template #default="scope">
                        <text-button @click="onLogContentDetail(scope.row)">查看</text-button>
                    </template>
                </el-table-column>
            </template>
        </table-extend>
        <div class="fixedDialog">
            <el-dialog v-model="dialogVisible" width="50%" :before-close="beforeClose">
                <template v-slot:title>
                    <dialog-title title="日志详情"></dialog-title>
                </template>
...
import { getLogList, getUserList } from "@/api/system";
import type { Pagination } from "@/components/core/tableExtend/type";
...
async function getusers() {
    ...
    if (res.code == 200) {
        users.value = res.data.records.map(m => ({ key: m.userId, value: m.userName }))
    }
}
