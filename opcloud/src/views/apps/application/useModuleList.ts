...
import { ResultUnit } from "@/shared/const";
...
export function useModuleList(
  ...
  function initSearchConfig() {
    ...
    searchForm.formConf.labelWidth = 100;
    ...
    if (componentList && componentList.length > 0) {
      componentList.forEach((item: DesignComponent) => {
        ...
        if (topItem.__config__.inSearch) {
          ...
          if (topItem.__config__ && topItem.__config__.defaultValue) {
            topItem.__config__.defaultValue = undefined;
          }
   ...
   * @return {*}
   */
  function initPageConfig(response, target: any) {
   ...
   */
  function getProcessProgressData(
    dataSource: any[],
    pageInfo: any,
    tableOption: any
  ) {
    ...
    dataSource.map((m) => {
      ...
      requestList.push(getProcPercentageData(params));
    });
    return Promise.all(requestList).then((reslist) => {
      for (let i = 0; i < reslist.length; i++) {
        ...
        if (res.code == 200) {
          ...
          if (!data) return;
          const row = tableOption.tableData.find((f) => f.id === data.rowId);
          if (!row) return;
          row.progressData = data.data || [];
        }
      }
    });
  }
   ...
   * @param target
   */
  function refreshTable(target: any) {
    // 全选来触发表格工序进度刷新
    target.value.tableRef.toggleAllSelection();
    // 必须用setTimeout清除
    setTimeout(() => {
      target.value.tableRef.clearSelection();
    }, 10);
  }
...
export function useFlowable() {
  ...
  async function flowableExcuteForm(
    item: any,
    ...
    if (item.eventCode == "IMPORT_WORK") {
      if (!currentFormData.FProRouteCode) {
      ...
      const importfunc = async () => {
        let params = {
          productionOrder: {
            FProRouteCode: currentFormData.FProRouteCode,
          },
        };
      ...
      if (
        tabTableData.length > 0 &&
        tabTableData[0].dataSource &&
        tabTableData[0].dataSource.length > 0
      ) {
        let obj;
        await confirmBoxAsync("是否覆盖原来的工艺流程", async () => {
          const resValue = await importfunc();
          obj = { resValue: resValue, boolValue: true };
        });
        return obj;
      } else {
        const resValue = await importfunc();
        const obj = { resValue: resValue, boolValue: true };
        return obj;
      }
    } else {
      let para = {
