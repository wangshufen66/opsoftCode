...
export default defineComponent({
  ...
  methods: {
    async fetchStacknBarData() {
      const url = `${this.host}/idcloud/dashboard/deviceRunning/getWorkShopDeviceRunningByTime?orgId=${this.orgId}&id=1711`;
      const url2 = `${this.host}/idcloud/dashboard/deviceRunning/getWorkShopDeviceRunningByTimeorgId=${this.orgId}&id=1712`;
    ...
    async fetchPieData() {
      const url = `${this.host}/idcloud/dashboard/deviceRunning/getWorkShopDeviceStatusRatio?orgId=${this.orgId}&type=shop&id=1711`;
      const res = await this.fetchFunc(url);
    ...
    async fetchPieDataNum() {
      const url = `${this.host}/idcloud/dashboard/deviceRunning/getWorkShopDeviceStatusRatio?orgId=${this.orgId}&type=shop&id=1712`;
      const res = await this.fetchFunc(url);
    ...
    drawPieCharNum(data: any) {
      const pieChartNum = echarts.init(document.getElementById('pie-container-num')!);
      let option = {
        tooltip: {
          trigger: 'item',
          formatter: '{a} <br/>{b}: {c}%',
    ...
    drawStackChart(data: any, data2: any) {
      ...
      const stackChartNum = echarts.init(document.getElementById('stack-container-num')!);
      //五轴加工中心柱状图设置
    ...
    drawBarChart(data: any) {
      ...
      data.list.forEach((i: any, index: any) => {
        xD.push(moment(i.day).format('MM-DD'));
        xD.sort()
        console.log(xD);
        dD.push();
