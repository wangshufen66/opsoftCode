  ...
  <div :class="rootClass">
    <ul :class="rootClass + '__wrapper'">
      <li :class="rootClass + '__item'">
        ...
        >
          <div
            :class="rootClass + '__imgcont'"
            style="display: flex; flex-direction: column; justify-content: center; width: 100%; text-align: center; border-bottom: 1px solid #f3f3f3;"
          >
            <i class="el-icon-plus" style="font-size: 32px"></i>
          </div>
          <label :class="rootClass + '__lbl'">{{blankTemplate.name}}</label>
        </div>
      </li>

      <li v-for="(template, index) in templatesLocal" :key="index" :class="rootClass + '__item'">
        <div
          :class="classThumbnail(tplSelected.hash, template.hash)"
          @click="onSelectImage(template)"
        >
          <div :class="rootClass + '__imgcont'">
            <!-- <img :src="template.imgUrl" :height="h" :width="w" :class="rootClass + '__img'" /> -->
            <el-image
              :src="template.imgUrl"
          ...
          <label :class="rootClass + '__lbl'">{{template.config.title}}</label>
        </div>
      </li>
    </ul>
    ...
    <div v-show="imgPreview">
      <label>大屏预览</label>
      <el-image
        :src="tplSelected.imgUrl"
        :height="h"
        :width="w"
        :class="rootClass + '__img'"
        fit="fill"
        :preview-src-list="[tplSelected.imgUrl]"
      ></el-image>
      <!-- <img :src="tplSelected.imgUrl" :height="h" :width="w" :class="rootClass + '__img'" /> -->
    </div>
  </div>
</template>
...
.template-list__imgcont img,
.template-list__imgcont span {
