<template>
  ...
  >
    <v-echart :options="options" autoresize class="ffff" />
  </div>
</template>
...
const WIDGET_NAME = "BarsChart";
export default {
  name: WIDGET_NAME,
  group: "chart",
  icon: require("./icon/thumb-barschart.png"),
  title: "分组柱状图",
  panel: stylec,
  setting: {
    type: WIDGET_NAME,
    ...
    seriesBarGap: false, // 柱体间距
    seriesColors: [],

    legend: [],

    dataAPI:
      "https://easy-mock.com/mock/5c7ce20ccdc04f0e04185d9b/example/mock_copy_1560491079589/charts", // API拉取地址
  ...
  computed: {
    ...
    dataSeries() {
      if (
        ...
        const orign = this.dynamicData[this.val.keyPrimary][this.val.keyTarget][
          this.val.keyYAxis
        ];
        if (this.$vpd.state.uuid === this.val.uuid) {
          ...
          if (datalen > this.val.lgArr.length) {
            ...
            let paramDel = {
              name: ["seriesBarWidth", "seriseRadiuses", "lgArr"],
              value: datalen,
            };
        ...
        return orign.map((item, i) => {
          return {
            type: "bar",
            name: item.name,
            barWidth: this.val.seriesBarWidth[i],
            // barGap: this.val.seriesBarGap, // 柱体间隔
            itemStyle: {
              normal: {
                barBorderRadius: [
                  this.val.seriseRadiuses[i][0] ||
                    this.val.seriseRadiuses[i][0] + 1,
                  this.val.seriseRadiuses[i][1] ||
                    this.val.seriseRadiuses[i][1] + 1,
                  this.val.seriseRadiuses[i][2] ||
                    this.val.seriseRadiuses[i][2] + 1,
                  this.val.seriseRadiuses[i][3] ||
                    this.val.seriseRadiuses[i][3] + 1,
                ], // 柱形图圆角
