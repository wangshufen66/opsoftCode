...
<template>
  <div
    ...
    <div style="width:98%;margin-top: -18px;display: flex;justify-content: flex-end;">
      <el-tag
        v-for="(cellTypeVal,cellTypeKey,cellTypeIndex) in cellTypesExplain"
        :key="cellTypeIndex"
        :type="cellTypeKey=='normal'?'':cellTypeKey"
        size="mini"
        effect="dark"
        class="color-box-explain">
        {{ cellTypeVal }}
      </el-tag>
...
import defaultTableCards from './default-table-cards'

export default {
  name: 'CustomReportTableModule',
  components: { defaultTableCards },
  props: {
    api: { // 后台请求接口的路由地址
      type: String,
      default() {
