<template>
  <div class="fixedDialog">
    <el-dialog :modelValue="visible" @closed="closeDialog" destroy-on-close width="800px">
      <template v-slot:title>
        <dialog-title :title="`模型设计 - ${formModel.modelName}`"></dialog-title>
      </template>
      <div class="top-params">
        ...
        <p>
          ...
          <span
            class="text"
            :title="formModel.modelSpecification"
          >{{ formModel.modelSpecification }}</span>
        </p>
      </div>
      <div class="line">
        <div>参数</div>
  ...
  setup(props, { emit }) {
      ...
      if (res.code == 200) {
        ...
        result.infoList.forEach((f, index) => {
          if (f.modelKey !== "keepDecimalPlaces") {
            ...
            if (f.fieldSource && f.fieldSource.indexOf("#") > -1) {
              const ids = f.fieldSource.split("#")[0];
              f.fieldSource = ids
                .split(",")
    ...
    async function onSubmitForm() {
      ...
      for (let i = 0; i < params.infoList.length; i++) {
        ...
        // }
        Array.isArray(params.infoList[i].fieldSource) &&
          (params.infoList[i].fieldSource =
            params.infoList[i].fieldSource.join(","));
      }
