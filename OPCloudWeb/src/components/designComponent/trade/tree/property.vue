...
<template>
  <el-form :model="formModel" :rules="formRules" ref="formRef" label-width="110px" size="small">
    ...
    <div class="switch-wrap">
      <el-col :span="12">
        <el-form-item label="手风琴">
          <el-switch v-model="componentCfg.__attr__.accordion" />
        </el-form-item>
      </el-col>
    </div>
    <property-mobile-config :componentCfg="componentCfg"></property-mobile-config>
    <div class="divider-wrap tree-list-header">树层级</div>
    <div class="tree-item-list">
      <div class="tree-item" v-for="(element, index) in componentCfg.options" :class="{ active: currentLeveId === element.id }" @click="onTreeItemClick(element.id, index)">
        <div class="move-icon">
    ...
    <div class="property-additem">
      <el-button class="btn-operate" icon="el-icon-circle-plus-outline" type="text" @click="addTreeItem" v-show="currentLeveId">添加层级</el-button>
    </div>
    <div v-show="currentLeveId">
      ...
      <el-form-item label="关联表" prop="pageTableName">
        <el-select v-model="selectTree.pageTableName" clearable filterable @change="onTableChange">
          <el-option v-for="table in treeList[currentLevel]" :key="table.id" :value="table.tableName" :label="table.tableRemark"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="关联字段" prop="fieldCode">
        <el-select v-model="selectTree.fieldCode" clearable filterable>
          <el-option v-for="fied in fiedList[currentLevel]" :key="fied.id" :value="fied.fieldName" :label="fied.fieldRemark"></el-option>
        </el-select>
      </el-form-item>
    </div>
  </el-form>
...
export default {
  name: "TreeProperty",
  props: {
    componentCfg: Object as PropType<TreeComponent>,
  },
  setup(props, { emit }) {
    const activeName = ref<string>("0");

    const lastNode = ref<any>(undefined);
    ...
    const selectTree = ref<any>(props.componentCfg?.options[0]);

    /**当前的层级id*/
    const currentLeveId = ref<number | string | undefined>(
      props.componentCfg?.options[0].id
    );
    /**当前的层级*/
    ...
    watch(
      ...
      () => {
        // selectTree.value = props.componentCfg?.options[0];
        // currentLeveId.value = props.componentCfg?.options[0].id;
    ...
    watch(currentLeveId, (value) => {
      if (!value) return;
      selectTree.value = props.componentCfg?.options?.find(
        (f) => f.id === value
      );
    ...
    function addTreeItem() {
      const flag =
        currentLeveId.value ===
        props.componentCfg?.options[optionLength.value - 2].id;
      if (!flag) {
        ElMessage.warning("请选择最大的层级后再添加层级");
        return;
      }

      if (selectTree.value.fieldCode === "") {
        ElMessage.warning("请选择需要绑定的字段后再添加层级");
        return;
      }
      if (treeList.value[currentLevel.value + 1].length === 0) {
        ElMessage.warning("暂无可选择下级表单");
        return;
      }
      init();
      const id = +new Date();
      if (optionLength.value > 4) {
        ElMessage.warning("最多添加四级");
        return;
      }
      ...
      props.componentCfg?.options.push(params);
      ...
      if (mitter.emitter.all.get("design-tree-node-add")) {
        ...
        mitter.emitter.emit("design-tree-node-add", {
          data: newNode,
          parentKey: lastNode.value.id,
        });
        lastNode.value = newNode;
      }
    }
    /**删除当前层级*/
    function onDeleteTreeItem(index: number) {
      ...
      const tempArr = deepClone(props.componentCfg?.options);
      ...
      props.componentCfg?.options.splice(index);
    ...
    async function onTreeItemClick(id: number | string, index: number) {
      if (!treeList.value[index]) {
        ElMessage.warning(`请先点击层级${index}`);
        return;
      }
      currentLeveId.value = id;
      ...
      //获取上一级的表名
      const tableName = props.componentCfg?.options[index - 1].pageTableName;
    ...
    /**初始化获取配置字段长度和最后的结点*/
    function init() {
      optionLength.value = props.componentCfg?.options.length + 1;
      const data = props.componentCfg?.options[optionLength.value - 2];
      data &&
        (lastNode.value = {
          id: data.id,
          name: data.name,
