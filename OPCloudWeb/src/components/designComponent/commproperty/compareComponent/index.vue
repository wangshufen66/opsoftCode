  ...
  <div v-if="componentCfg.__config__.verifyComponent">
    ...
    <el-form-item label="校验组件">
      <el-select v-model="componentCfg.__config__.verifyComponent.tag" clearable>
        <el-option v-for="component in componentList" :key="component.componentId" :value="component.componentId" :label="component.__form__.label" @change="addCompareItem"></el-option>
      </el-select>
    </el-form-item>
    <el-form-item label="校验类型">
      <el-select v-model="componentCfg.__config__.verifyComponent.type" clearable @change="addCompareItem">
        <el-option v-for="(item,index) in regTypeList" :key="index" :value="item.value" :label="item.lable"></el-option>
      </el-select>
    </el-form-item>
  </div>
</template>
...
export default {
  ...
  setup(props) {
    ...
    // console.log(props.componentCfg)
    /** 添加校验 */
