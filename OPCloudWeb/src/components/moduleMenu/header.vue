  ...
  <div class="module-header">
    ...
    <el-breadcrumb class="breadcrumb-separator">
      <template v-for="item in paths" :key="item.name">
        <el-breadcrumb-item v-if="!item.path" class="path-item">
          <!-- <i :class="item.icon || 'el-icon-menu'"></i> -->
          {{ item.name }}
        </el-breadcrumb-item>
        <el-breadcrumb-item v-else class="path-item" :to="item.path" replace>
          <!-- <i :class="item.icon || 'el-icon-menu'"></i> -->
          {{ item.name }}
        </el-breadcrumb-item>
      </template>
...
.module-header {
  ...
  border-bottom: 1px solid $border-color-light1;
}
.breadcrumb-expand {
  margin-right: 15px;
  color: $text-color-light3;
  cursor: pointer;
}
.path-item {
  font-weight: 400;
