  ...
  <div>
    <el-form :inline="true" size="small" label-width="90px">
      <el-form-item label="场景查询">
        <el-select v-model="sceneName" placeholder="请选择" clearable @change="onchangeoption">
          <el-option-group v-for="group in groupOptions" :key="group.id" :label="group.label">
            <el-option
    ...
    <el-dialog v-model="dialogVisible" width="900px">
      ...
      >
        ...
        >
          ...
          <!--   筛选条件 > = <...  -->
          <el-select v-model="item.symbolCode" placeholder="请选择条件" style="margin: 0 20px">
            <el-option
          ...
          <template v-if="item.valueType !== 'date'">
            ...
            <el-input-number v-if="item.valueType === 'number'" v-model="item.value1" clearable></el-input-number>
            <el-time-picker v-if="item.valueType === 'time'" v-model="item.value1"></el-time-picker>
          </template>
    ...
    <el-dialog v-model="manageVisible" width="600px">
      ...
      <table-extend v-bind="tableOption">
        <template #operatColumn>
          <el-table-column label="操作" fixed="right" min-width="100" width="150px">
            <template #default="scope">
              <el-button type="text" size="small" @click="onEditScene(scope.row)">编辑</el-button>
              <el-button type="text" size="small" @click="onDeleteScene(scope.row, scope.$index)">删除</el-button>
            </template>
          </el-table-column>
        </template>
      </table-extend>
