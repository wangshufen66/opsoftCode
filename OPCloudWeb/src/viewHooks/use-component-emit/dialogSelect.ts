...
export function loadDialogSelectData(dialogSelectCfg: any) {
  ...
  Promise.all([
    ...
    const columnModel: string[] = searchColumns.map((m) => m.prop) || [];
    // 获取显示的组件配置
    ...
    initComponentData(tableComponents).then(() => {
      // 根据组件类型，处理表格数据
      transformDataSourceOfTableData(tableComponents, tableData.records || []);
  ...
  for (let i = 0; i < componentList.length; i++) {
    ...
    if (componentId === relComponentCfg.componentId) {
      if (component.componentTag === "designInput") {
        ...
        // 关联字段的值
        const relValue =
          row[`${relComponentCfg.field}_key`] || row[relComponentCfg.field];
        // 联动组件字段=关联字段的值
        const filter = {
