<template>
  <div class="dictionary-list">
    <div class="list-left">
      ...
      >
        <template #operatColumn>
          <el-table-column label="操作" min-width="120" fixed="left">
            <template #default="scope">
              <el-button type="primary" plain size="small" @click.stop="onTypeEdit(scope.row)">编辑</el-button>
              <el-button
                type="danger"
                size="small"
                plain
    ...
    <div class="list-right">
      ...
      >
        <template #operatColumn>
          <el-table-column label="操作" min-width="120" fixed="left">
            <template #default="scope">
              <el-button type="primary" plain size="small" @click="onInfoEdit(scope.row)">编辑</el-button>
              <el-button
                type="danger"
                size="small"
                plain
    ...
    <el-dialog v-model="dialogVisible" :title="formTitle" @closed="dialogClosed">
...
async function getTypeist() {
  ...
  const res = await getDictionaryType(params);
  const { code, data } = res;
  if (code === 200) {
...
async function onTypeDelete(id: string) {
  confirmBox('删除该数据', async () => {
    const res = await delDictionaryType({ ids: id });
    if (res.code === 200) {
...
function onInfoDelete(id: string) {
  confirmBox('删除该数据', async () => {
    const res = await delDictionaryInfo({ ids: id });
    if (res.code === 200) {
      messageAlert('success', '删除成功')
      getInfoList();
    }
  ...
  formRef.value.validate(async (val) => {
    ...
    if (isType.value) {
      ...
      if (isEdit.value) {
        ...
        const res = await editDictionaryType(para);
        saveLoading.value = false;
        if (res.code === 200) {
          getTypeist();
        ...
        //类型增加
        const res = await addDictionaryType(para);
        saveLoading.value = false;
        if (res.code === 200) {
          getTypeist();
          dialogVisible.value = false;
      ...
      if (isEdit.value) {
        ...
        const res = await editDictionaryInfo(params);
        saveLoading.value = false;
        if (res.code === 200) {
          getInfoList();
          dialogVisible.value = false;
        ...
        params.dictionaryId = typeActive.value;
        //信息增加
        const res = await addDictionaryInfo(params);
        saveLoading.value = false;
        if (res.code === 200) {
          getInfoList();
          dialogVisible.value = false;
...
@import "@/styles/modules/system/dictionary.scss";
</style>
