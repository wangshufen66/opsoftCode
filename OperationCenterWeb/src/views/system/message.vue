  ...
  <div class="newslist">
    <div class="pageList-top">
      <el-input
        placeholder="请输入消息名称"
        type="text"
        v-model.trim="topForm.title"
        @keyup.enter="onSerach"
        clearable
      ></el-input>

      <el-select placeholder="请选择发布状态" v-model="topForm.status" clearable>
    ...
    >
      ...
      <!-- 等级 -->
      <template #col_level="{ data }">
        <span>{{ data.level === 0 ? "公司通知" : data.level === 1 ? "部门通知" : data.level === 2 ? "人事通知" : "" }}</span>
      </template>
      <!--  分类-->
      <template #col_type="{ data }">
        <span>{{ data.type === 0 ? "紧急信息" : data.type === 1 ? "重要信息" : data.type === 2 ? "请知晓" : "" }}</span>
      </template>
      <!-- 位置 -->
      ...
      <template #col_status="{ data }">
        <span>{{ data.status == 0 ? '未发布' : '已发布' }}</span>
      </template>
      ...
      <template #operatColumn>
        <el-table-column label="操作" min-width="280">
          <template #default="scope">
            ...
            <el-button type="primary" size="small" @click="onDelRow(scope.row)">删除</el-button>
    ...
    <el-dialog v-model="visible">
      ...
      <el-form :model="dataForm" ref="addPageRef" :rules="formRules" label-width="120px">
        ...
        <el-form-item label="分类" prop="type">
          <el-select v-model="dataForm.type" placeholder="请输入分类" style="width: 100%">
            <el-option label="公司通知" :value="0"></el-option>
            <el-option label="部门通知" :value="1"></el-option>
            <el-option label="人事通知" :value="2"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="等级" prop="level">
          <el-select v-model="dataForm.level" placeholder="请选择" style="width: 100%">
            <el-option label="紧急信息" :value="0"></el-option>
            <el-option label="重要信息" :value="1"></el-option>
            <el-option label="请知晓" :value="2"></el-option>
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="位置" prop="locationedList">
          <el-select v-model="dataForm.locationedList" multiple style="width: 100%;">
            <el-option label="登录首页" :value="1"></el-option>
            <el-option label="通知中心" :value="2"></el-option>
          </el-select>
        </el-form-item>
      </el-form>
      <template #footer>
...
import tableColums from '@/config/tableConfig/messageList';
import {
  getNewsList,
  getNewsInfoById,
  delNewsInfoById,
...
const formTitle = computed(() => {
  return isEdit.value ? '编辑消息' : '新建消息页';
});
...
function pageEdit(row: any) {
  ...
  dataForm.value.level = row.level;
...
const getPageList = () => {
  ...
  getNewsList({ ...topForm, ...pagination }).then((res: any) => {
    if (res.code == 200) {
